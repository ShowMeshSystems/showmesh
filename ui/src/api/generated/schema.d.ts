...
export interface components {
    schemas: {
        ...
        NightBackgroundAudio: {
            ...
            /** @description The background-audio ceiling the RUNNING session pinned when it started (resting.backgroundAudio.maxGainDb on the session's own pinned configRevision) - never the value night.session's config currently holds, which can differ across a later revision (owner ruling 2026-08-28: the useful number during a show is the limit the session pinned, not the limit currently configured). Populated ONLY while the top-level state is one of preshow, transition-to-show, live, transition-to-resting, resting-intershow, end-of-night-resting, or fading-out. Absent or null in every other state (including inactive, preparing, and stopped, so the last night's ceiling is never reported as live once the night has ended or before it has begun), and null when the pinned revision configures no background audio at all, in which case `reason` says so. Never a fallback to the currently configured value. */

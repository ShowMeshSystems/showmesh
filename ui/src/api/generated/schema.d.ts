...
export interface paths {
    ...
    "/integrations/fpp/playlists/{playlistId}/readiness": {
         ...
         * @description Open under `observation:read`, matching the reconciliation route above. Reports whether every one of §6's ordered conditions holds, and the exact failing one when it does not. §6 originally specified five, closed; Lane 16 opened the vocabulary and this route currently checks eight -- see internal/coordinator/fppreconcile/readiness.go's own doc comment for the current, authoritative list, since further lanes are already adding to it. The original five: a definition is stored for the binding's (instanceUuid, playlistHash), every entry's (section, position) exists in that definition with matching filenames, every referenced Cue exists and belongs to the same Show, and the latest accepted observation (when one exists) carries the same playlistHash. Refuses with `400` for a playlist whose runner is not `fpp`: §6 readiness is an FPP-specific concept. Read-only: this route never activates anything.
         */

...
export interface paths {
    ...
    "/config/night.session/{id}": {
         ...
         * @description Requires `config:write` (admin only). FPP alone authorizes and schedules a night session (ADR-038): a calendar field or a hand-entered rest-duration field anywhere in the payload is rejected. `siteControl`/`interlocks` (RESTING-MODE.md §10) are accepted on write, returned on read, and enforced against a lifecycle command - the whole block's absence is valid and is NOT degraded. showPlaylist.fppInstanceId and resting.fppInstanceId must name configured FPP instances; resting.timelineAsset and every backgroundAudio item must resolve to a current asset (ADR-028); every cue's action must name an existing show.action object in the same show. This is a FULL REPLACEMENT: an omitted optional block (backgroundAudio) is left unconfigured, never wiped by an absent key on top of a required one. Optionally carries `If-Match`/`If-None-Match` (opt-in; see those parameters): refused with `409` when the precondition names a revision that is no longer current.
         */
    ...
    "/night/commands/{command}": {
         ...
         *
         *     `skipEnterShowLead` (optional request body field) is honored only by `start-night`, for the operator starting a night later than planned: the first show of the night otherwise leads its own launch by the largest negative `offsetMs` among its `enterShow` cues (RESTING-MODE.md §7.1), the same lead every later cycle already holds for. Setting it true launches immediately instead, skipping that wait; an `enterShow` announcement cue still dispatches. Every other command ignores the field.
         *
...
export interface components {
    schemas: {
        ...
        /** @description What happened to one dispatched (or replayed) audio.session.* command. outcome is never "successful" merely because the publish to the node succeeded (ADR-003): it is decided from the node's own evidence, collected after its own dispatch. A SUCCESSFUL HTTP 200 response can still carry `outcome: "unconfirmable"` - this is a real, expected outcome whenever the node's own evidence does not corroborate the command, never an error and never a transport failure. Reading only the HTTP status code is not enough to know whether the command worked; read `outcome`. */
        ...
        ConfigAudioSettingsPayload: {
            ...
            /** @description How long a session takes to fade DOWN to duckTargetGainDb once a higher-priority session starts ducking it. Fast by design: an announcement is already talking over the bed once the duck starts. */
            duckFadeDurationMs: number;
            /** @description How long a session takes to fade back UP once its last ducker releases it. Slower than duckFadeDurationMs by design: an abrupt return to full level once nothing is talking over the bed reads as jarring in a way the duck-down does not. */
            duckRestoreFadeDurationMs: number;
            /** @enum {string} */
        ...
        /**
         * @description The "audio.node" configuration kind's decoded payload (ADR-018/ADR-039): the body PUT /config/audio.node/{id} accepts (a full replacement: every field the write omits is cleared, and every required field must be non-null and non-empty), and the "payload" member of GET /config/audio.node/{id}'s response. `programRoute` and `ltcRoute` name discovered output routes (device identities the node itself reported) and, when both are given, MUST name the same route, so timecode cannot drift against the music: two different route names are refused. `programChannels` is the ordered, distinct, 1-based channel indices on that route carrying program audio ([1, 2] for reference stereo, [1] for mono); `ltcChannel` is the 1-based index carrying LTC and must not appear in `programChannels`.
         *
         ...
         *
         *     `localClockOverride` names this node's local clock, the sample clock of the interface its program audio leaves through. It is optional: absent, the local clock is derived from `programRoute`, which is the answer on every single-interface node. Name it only for an arrangement the node cannot see, such as two interfaces sharing external word clock. `clockDomain` and `clockDomainProvenance` were this field's required predecessors; a stored object or a write that still carries them is accepted, both are ignored, and they are gone from the object on its next write. `role` (ADR-045) is one of `program`, `program+ltc`, or `zone`; optional on the wire, and absent decodes to `program+ltc` - the role every pre-ADR-045 audio.node object already implicitly held, since an installation had exactly one and it always carried both program and LTC. At most one audio.node across the installation may carry `program+ltc` at a time (ADR-018: one LTC emitter); a second is refused, naming both node ids. `zone` is the operator's own name for the independent speaker zone this node drives, present only when `role` is `zone` - refused on any other role, since an ignored field would read as an applied one.
         *
        ...
        ConfigAudioNode: {
            ...
            ltcChannel?: number;
            /** @description Optional. Names this node's local clock explicitly; absent, it is derived from programRoute (ADR-052). */
            localClockOverride?: string;
            /**
        ...
        CueActivationNodeOutcome: {
            ...
            unalignedReason?: string;
            /**
             * @description ADR-051 decision 6. Present only when this activation had an audio output and no ADR-049 shared start instant was ever attempted (a Cue reaching one node, the ordinary case). "multisync" when the coordinator's own evidence showed a MultiSync START packet already started this Cue's audio before cue.activate was dispatched; "coordinator" when the fallback window elapsed first.
             * @enum {string}
             */
            startTrigger?: "multisync" | "coordinator";
            /** @description The FPP sequence filename a MultiSync START answered. Present only alongside startTrigger "multisync". */
            triggerSequenceFilename?: string;
            /**
             * Format: int64
             * @description The MultiSync START packet's arrival on the node's own media clock. Present only alongside startTrigger "multisync".
             */
            triggerArrivalNs?: number;
            /** @description The fixed lead applied after arrival before the first sample. Present only alongside startTrigger "multisync". */
            startLeadMs?: number;
            /** @description True when this node's prepare ran on the OPEN packet instead of ahead of time. Present alongside either startTrigger value. */
            preparedLate?: boolean;
        };
        ...
        /** @description The 200 response of POST /integrations/fpp/playlist-entry-observations (FPP-PLUGIN-COORDINATOR-CONTRACTS.md §1.6 step 10): what was decided, not an echo of the request. accepted is true when this observation changed the stored state; false only for an idempotent replay, which stores nothing and publishes nothing. replay is accepted's inverse. reconciliation and operatorInstruction are additive and optional, computed through the same resolution GET .../reconciliation uses (never a second one), and reported on a replay as well as on an accepted observation so a plugin that polls by re-posting is not blind on requests where nothing changed. Both are best effort and may be absent even for an instance that does have a verdict; a client must not read their absence as "no mismatch." */
        ...
        ConfigNightSessionBackgroundAudioItem: {
            ...
            show: string;
            sequence: string;
            /** @description Which registered copy of the file this item uses. When the bed's own targets (ADR-049) is absent or empty, this also decides where the item plays: items are not required to share one target, and every distinct target among them is its own node the bed plays on, independently. When the bed's own targets is non-empty, every listed node plays every item, and this field no longer decides where it plays. */
            target: string;
        };
        /** @description night.session.resting.backgroundAudio: a ShowMesh `background` playback session (RESTING-MODE.md §8) - a discriminated union of two mutually exclusive forms, distinguished by which of `items` and `mediaPlaylist` is present. The INLINE form (ConfigNightSessionBackgroundAudioInline) plays on every distinct node its own items[].target names (a list of target nodes, derived from the items themselves - there is no separate "which output(s)" field) unless `targets` overrides that, see `targets` below. The REFERENCE form (ConfigNightSessionBackgroundAudioReference) names a media.playlist object whose own items/repeat/resume/itemTransition/ gain/fade fields govern the bed instead - editing that object re-pins the bed on its next apply without writing this session again. Present only when the deployment configures background audio at all; its absence is valid and is not degraded. `targets` (ADR-049) applies identically to both forms. */
        ConfigNightSessionBackgroundAudio: components["schemas"]["ConfigNightSessionBackgroundAudioInline"] | components["schemas"]["ConfigNightSessionBackgroundAudioReference"];
        /** @description The INLINE form of resting.backgroundAudio: items plus repeat/resume/itemTransition/maxGainDb and, optionally, crossfadeMs/fadeOutMs/fadeInMs. `resume` and `itemTransition` are pinned against pkg/audio's vocabulary on Track C's branch (track-c/audio-node). crossfadeMs is required when itemTransition is "crossfade" and must be absent otherwise (server-side; not expressible here). maxGainDb must be <= 0. fadeOutMs and fadeInMs are the show-boundary fade pair: fadeOutMs fades the bed to silence before it is paused or stopped for a show, fadeInMs fades it back up to maxGainDb after resting returns. They must be configured together, or both omitted for an instant cut exactly as before this pair existed (server-side; not expressible here). mediaPlaylist is not permitted here - naming it selects the reference form instead. */
        ...
        /** @description The WRITE shape of resting.backgroundAudio: identical to ConfigNightSessionBackgroundAudio except that repeat is not required on the inline form - an absent repeat takes its documented default of "none". A body naming mediaPlaylist alongside any inline-only property, or naming neither form at all, is refused (server-side; not expressible here). */
        ConfigNightSessionBackgroundAudioWrite: components["schemas"]["ConfigNightSessionBackgroundAudioInlineWrite"] | components["schemas"]["ConfigNightSessionBackgroundAudioReferenceWrite"];
        /** @description The WRITE shape of the inline form: identical to ConfigNightSessionBackgroundAudioInline except that repeat is not required - an absent repeat takes its documented default of "none". crossfadeMs is still required when itemTransition is "crossfade" and must be absent otherwise (server-side; not expressible here). fadeOutMs and fadeInMs still must be configured together, or both omitted for an instant cut (server-side; not expressible here). mediaPlaylist is not permitted here - naming it selects the reference form instead. */
        ...
        /** @description The body of POST /nodes/{nodeId}/cue-catalog/deploy. Carries only an idempotency key and an override flag - the catalog itself is always THIS coordinator's own current resolution, never a request field. */
        CueCatalogDeployRequest: {
            /** @description Optional; a fresh key is minted server-side when omitted. A replayed key dispatches nothing and returns the original command's own result, flagged `replay: true`. */
            idempotencyKey?: string;
            /**
             * @description An operator's explicit acceptance of an H0.5 exclusive-claim conflict on the resolved catalog: when true AND the only refusal this deploy would otherwise give is that conflict, it deploys anyway instead of returning `409` (`cue-catalog-claim-conflict`), and records the acceptance scoped to exactly the confirmed revision - a later revision requires overriding again. It never bypasses any other refusal (no active show configured, or a genuine resolution error).
             * @default false
             */
            override?: boolean;
        };
        ...
        CueCatalogDeployResult: {
            ...
            acknowledgedRevision?: string;
            /** @description Non-empty exactly when this dispatch bypassed a refusal because the request carried `override: true` - every condition it accepted, so the operator sees exactly what they overrode. */
            overriddenConditions?: components["schemas"]["CueCatalogOverriddenCondition"][];
            /**
             * Format: date-time
        ...
        /** @description One refusal condition an operator explicitly accepted via `override: true`. `kind` is presently always `exclusive-claim-conflict` - the only overridable refusal this route wires today. */
        CueCatalogOverriddenCondition: {
            /** @enum {string} */
            kind: "exclusive-claim-conflict";
            cueA: string;
            cueB: string;
            /** @description The exact H0.5 claim string both Cues held (config.ShowCueClaim.String()). */
            claim: string;
        };
...
export interface operations {
    ...
    deleteAsset: {
        ...
        responses: {
            ...
            404: components["responses"]["ResourceNotFound"];
            405: components["responses"]["MethodNotAllowed"];
            /** @description This asset is pinned by id (`asset-pinned`): the current show.weatherdelay alert or a current show.action's announcement media names it directly. `detail` names what references it. */
            409: {
                headers: {
                    "ShowMesh-API-Version": components["headers"]["ShowMesh-API-Version"];
                    [name: string]: unknown;

...
describe('NightSessionActive', () => {
  it('renders "not configured" when nothing has ever been activated', async () => {
  ...
  it('renders the arm buttons disabled with a stated reason when config:write is not held', async () => {
    getNightSessionActiveConfig.mockResolvedValue(makeActiveConfigResponse('halloween-night'))
    getNightSessionActiveConfigRevisions.mockResolvedValue({
      serverTime: '2026-08-22T00:00:00Z',
      kind: 'night.session.active',
    ...
    const button = screen.getByRole('button', { name: 'Activate this session…' })

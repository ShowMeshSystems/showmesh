...
describe('Show Night', () => {
  ...
  it('renders each earlier cycle from its finished-cycle record, and the live one for the current cycle', () => {
    ...
    expect(cycleSteps[1]?.detail).toBe('Stopped')
    expect(cycleSteps[1]?.title).toBe('Check the coordinator log for cycle 2.')
  ...
  it('renders every finished-cycle outcome, including an earlier cycle with no record at all', () => {
    ...
    expect(cycleSteps[1]).toMatchObject({ status: 'stopped', detail: 'Stopped' })
    expect(cycleSteps[2]).toMatchObject({ status: 'error', detail: 'Interrupted' })
    expect(cycleSteps[3]).toMatchObject({ status: 'unknown', detail: 'Unconfirmed', title: 'Check the coordinator log for cycle 4.' })

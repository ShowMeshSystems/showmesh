...
import { Button, ButtonRow, Input, RuledStrip, StatusPair, Table, TableWrap, Workbench } from '../kit'
...
export function ResolumeControl() {
  ...
  const reportedDeckSelected = typeof reportedDeck?.value === 'string' && reportedDeck.value !== ''
  useEffect(() => {
    if (data === null || !gate.allowed || chosenDeck !== '' || reportedDeckSelected) return
    const first = data.decks[0]
    if (first === undefined) return
    setChosenDeck(first.name)
    run(() => selectResolumeDeck(first.name))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data, gate.allowed, chosenDeck, reportedDeckSelected])
  ...
  return <>
    ...
    <Workbench
      sideLabel="Resolume target and dispatch log"
      main={<>

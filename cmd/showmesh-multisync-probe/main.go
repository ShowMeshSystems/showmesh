...
func (c *capture) printSummary(w *summaryWriter, l *multisync.Listener, outPath string, stepTime time.Duration, cfg runConfig) {
	...
	if c.packetCount == 0 {
		...
		w.Println("This is itself a finding, not an error. Possible causes: FPP is not running")
		w.Println("or not playing anything; FPP's MultiSync transport (multicast/broadcast/")
		w.Println("unicast) is not reaching this host; this host is on a different L2 segment")
		w.Println("or VLAN with no route for the configured transport; a firewall is dropping")
		w.Println("UDP 32320; or -iface/-listen point somewhere traffic does not arrive. None")
		w.Println("of RES-002's five open items are answered by this run.")
	}

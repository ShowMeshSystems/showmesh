...
Module path is `github.com/showmeshsystems/showmesh`. Step 0 (foundation: scaffold, Docker image, Compose bundle, CI, minimal coordinator binary) and Step 1 (`pkg/multisync` plus the bench probe) are complete and verified locally. RES-002 is still L1: nothing has been run against a real FPP player. The repository is pushed to a private remote and CI runs on a real GitHub runner; its first run caught a Linux-only `SO_REUSEADDR` socket behavior that passes on macOS, which is why a socket claim verified only on macOS is not verified for this project.

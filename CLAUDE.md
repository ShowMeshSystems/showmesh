...
**Step 5 is specified in BUILD-PLAN and not started**: real FPP signals on the dashboard (pixel current, playback state, controller and network health, FPP MQTT ingestion), collected read-only from the deployed fleet whose addresses and versions are in `docs/reference-installation.md`. Those hosts are the operator's live display: **read-only, never a write, a command, a restart, or a settings change.**

Beyond Step 5 the sequencing is structural: **every remaining roadmap item that does something rather than shows something is a write operation, and ADR-021 rule 5 bars the first write endpoint** until a superseding ADR decides authenticated identities, authorization by target and action, audit attribution, the MQTT control plane's own authorization, and a browser session model. That record is the critical path out of Phase 0. Also available without it: RES-008's configuration model, and `pkg/pjlink` at L1.

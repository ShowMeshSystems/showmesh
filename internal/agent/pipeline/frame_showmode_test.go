...
func startCoverageGapWriter(t *testing.T, showMode ShowModeSource) (*Supervisor, *fakeProcess, *FrameWriter) {
	...
	fw, err := NewFrameWriter(sup, surfaceID, source, tl, "seq.fseq", 0, gapChannelCount, surfaceWidth, 1, IdleOutputBlack, showMode, nil, testLogger{})
	if err != nil {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			fw, err := NewFrameWriter(sup, surfaceID, source, tl, "seq.fseq", 0, gapChannelCount, surfaceWidth, 1,
				IdleOutputBlack, newFakeShowMode(tc.behavesAsShow), nil, testLogger{})
			if err != nil {
				t.Fatalf("NewFrameWriter: %v", err)
			}
			ctx, cancel := context.WithCancel(context.Background())
...
func TestNewFrameWriterRefusesEmptySource(t *testing.T) {
	...
	if _, err := NewFrameWriter(sup, surfaceID, source, &fakeTimelineSource{}, "seq.fseq", 0, 8, 8, 1, IdleOutputBlack, nil, nil, testLogger{}); err == nil {
...
func TestNewFrameWriterReportsCoercedIdleOutput(t *testing.T) {
	...
	if _, err := NewFrameWriter(sup, surfaceID, source, &fakeTimelineSource{}, "seq.fseq", 0, 8, 8, 1, "strobe", nil, nil, logger); err != nil {
		t.Fatalf("NewFrameWriter: %v", err)
	}
	...
	if _, err := NewFrameWriter(sup, surfaceID, source, &fakeTimelineSource{}, "seq.fseq", 0, 8, 8, 1, IdleOutputBlack, nil, nil, quiet); err != nil {
		t.Fatalf("NewFrameWriter: %v", err)
	}

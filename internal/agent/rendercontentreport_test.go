...
func publishAndDecodeRenderReport(t *testing.T, sup *pipeline.Supervisor, store *pipeline.AssignmentStore) mqttproto.RenderPayload {
	...
	defer cancel()
	publishOneRenderReport(ctx, pub, "showmesh/nodes/media-03/observed/render", "media-03", sup, store, nil, newMultiSyncStatus(), newFPPConnectHTTPStatus(), newTestFPPConnectHeldStore(t), time.Now, discardLogger())

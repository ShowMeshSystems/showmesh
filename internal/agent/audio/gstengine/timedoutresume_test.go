...
// up its clock/basetime negotiation during the original Start), so
// startTimeoutProbe's single 200us value never lands inside Resume's
// much narrower window; sweeping 1-20us per trial reliably catches it
// instead of chasing one magic number.
...
// the same one either way. Each trial pauses a fresh branch (Resume's
// precondition) before the timed-out Resume, sweeping resumeTimeoutProbe
// across the trial range to land inside the window.
	...
	for i := 0; i < trials; i++ {
		...
		if errors.Is(resumeErr, context.DeadlineExceeded) {
			...
			if state != pkgaudio.StatePlaying && !frozen {
				...
				// even though the branch never actually resumed after
				// Pause.
				buggy++
			}
		}
...
// Resume fix: Resume's flushing seek is issued before its ctx deadline
// can fire, and an abandoned seek may still land arbitrarily late with
// no way to know when, so the branch must be marked errAnchorUnknown

...
// mutation target: duckOneLocked's `if len(t.duckedByAll) == 0` guard on
// capturing preDuckGain, and removeDuckerLocked's restore. This is the
// compounding proof, and the reason the coordinator no longer sends its

...
func newTestRenderOperations(sup *pipeline.Supervisor, store *pipeline.AssignmentStore, dir string, clock *fakeClock) *renderOperations {
	return newRenderOperations(sup, store, pipeline.NewHoldBlackStore(dir), dir, multisync.NewTimeline(clock.now, multisync.Config{}), nil, "", discardLogger())
}
...
func TestHandleMessageRenderSurfaceApplyConfirmed(t *testing.T) {
	...
	h := newCommandHandler(testNodeID, dir, "", nil, renderOps, nil, nil, nil, nil, nil, nil, nil, nil, clock.now, discardLogger())
...
func TestHandleMessageRenderTriggerSignalsOnlyForRenderActions(t *testing.T) {
	...
	h := newCommandHandler(testNodeID, dir, "", nil, renderOps, trigger, nil, nil, nil, nil, nil, nil, nil, clock.now, discardLogger())
	pub := newFakePublisher()
...
func TestHandleMessageRenderTransportProbeFiresRenderTrigger(t *testing.T) {
	...
	h := newCommandHandler(testNodeID, dir, "", nil, renderOps, renderTrigger, nil, nil, nil, nil, nil, nil, nil, clock.now, discardLogger())
	pub := newFakePublisher()

...
func TestRunMultiSyncListenerBindFailureSetsStatus(t *testing.T) {
	...
	runMultiSyncListener(ctx, "test-node", addr, "", timeline, status, newFPPConnectState(), discardLogger())
...
func TestRunMultiSyncListenerSuccessSetsStatusListening(t *testing.T) {
	...
	go func() {
		defer close(done)
		runMultiSyncListener(ctx, "test-node", "127.0.0.1:0", "", timeline, status, newFPPConnectState(), discardLogger())
	}()

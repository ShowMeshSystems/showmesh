...
func TestRunRenderReportPublishesOnTick(t *testing.T) {
	...
	go func() {
		defer close(done)
		runRenderReport(ctx, pub, "media-03", sup, pipeline.NewAssignmentStore(t.TempDir()), newMultiSyncStatus(), time.Now, ticks, nil, discardLogger())
	}()
...
func TestRunRenderReportPublishesOnTrigger(t *testing.T) {
	...
	go func() {
		defer close(done)
		runRenderReport(ctx, pub, "media-03", sup, pipeline.NewAssignmentStore(t.TempDir()), newMultiSyncStatus(), time.Now, nil, triggered, discardLogger())
	}()

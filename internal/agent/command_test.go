...
func TestHandleMessageAgentEchoConfirmed(t *testing.T) {
	...
	h := newCommandHandler(testNodeID, t.TempDir(), "", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, clock.now, discardLogger())
...
func TestHandleMessageMalformedPayloadDropsWithNoPublish(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			h := newCommandHandler(testNodeID, t.TempDir(), "", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, clock.now, logger)
			pub := newFakePublisher()
...
func TestHandleMessageAssetFetchEndToEnd(t *testing.T) {
	...
	h := newCommandHandler(testNodeID, dir, "", trigger, nil, nil, nil, nil, nil, nil, nil, nil, nil, clock.now, discardLogger())
	pub := newFakePublisher()
...
func TestHandleMessageAssetFetchFailureCarriesReasonAndSignal(t *testing.T) {
	...
	dir := t.TempDir()
	clock := &fakeClock{t: time.Date(2026, 8, 16, 12, 0, 0, 0, time.UTC)}
	h := newCommandHandler(testNodeID, dir, "", nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, clock.now, discardLogger())
	pub := newFakePublisher()

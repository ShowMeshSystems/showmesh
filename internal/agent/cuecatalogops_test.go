...
func TestCuecatalogDeployIsRegisteredWhenCatalogStoreProvided(t *testing.T) {
	store := heldcatalog.NewFileStore(t.TempDir())
	ops := newOperationRegistry(testNodeID, t.TempDir(), "", nil, nil, nil, store, nil, nil, nil, discardLogger())
...
func TestCuecatalogDeployIsAbsentWhenNoCatalogStore(t *testing.T) {
	ops := newOperationRegistry(testNodeID, t.TempDir(), "", nil, nil, nil, nil, nil, nil, nil, discardLogger())
	if _, ok := ops["cuecatalog.deploy"]; ok {
...
func TestHandleMessageCuecatalogDeployConfirmed(t *testing.T) {
	...
	h := newCommandHandler(testNodeID, dir, "", nil, nil, nil, nil, nil, nil, store, nil, nil, nil, clock.now, discardLogger())

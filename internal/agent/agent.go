...
func Run() int {
	...
	// Set here, after conn exists, rather than at audioRebuilder's own
	// construction above: installAudioCapabilityRepublish needs a live
	// Publisher, which does not exist yet at that point.
	installAudioCapabilityRepublish(audioRebuilder, sigCtx, conn, cfg, bootID, startedAt, logger)

...
// TestRunnerNextNudgeAt is Step 9's own required proof for [Runner.NextNudgeAt]:
// unknown before any [Add] call for that id, the zero Time (accept
// immediately) before any [Nudge] call, and last-nudge-plus-window after
// one — read under the SAME fake clock [WithNudgeClock] gives [Nudge]
// itself, so the two methods' notion of "now" cannot silently disagree.
func TestRunnerNextNudgeAt(t *testing.T) {
	var clockMu sync.Mutex
	fakeNow := time.Unix(1_700_000_000, 0)
	clock := func() time.Time {
		clockMu.Lock()
	...
	r := NewRunner(&fakeSink{}, testLogger(), WithNudgeMinInterval(2*time.Second), WithNudgeClock(clock))

	if _, ok := r.NextNudgeAt("test"); ok {
		t.Error(`NextNudgeAt("test") ok = true before Add was ever called for it, want false`)
	}

	r.Add(&countingCollector{id: "test"}, time.Hour)

	next, ok := r.NextNudgeAt("test")
	if !ok {
		t.Fatal("ok = false for a registered id, want true")
	}
	if !next.IsZero() {
		t.Errorf("NextNudgeAt before any Nudge call = %v, want the zero Time (accept immediately)", next)
	}

	if !r.Nudge("test") {
		t.Fatal("Nudge = false, want true (no prior nudge to rate-limit against)")
	}
	next, ok = r.NextNudgeAt("test")
	if !ok {
		t.Fatal("ok = false after a successful Nudge, want true")
	}
	want := fakeNow.Add(2 * time.Second)
	if !next.Equal(want) {
		t.Errorf("NextNudgeAt after one Nudge = %v, want %v (last nudge + the configured window)", next, want)
	}

	// A second Nudge inside the window is suppressed and must NOT move
	// NextNudgeAt's answer — it reports when the FIRST accepted nudge's own
	// window ends, not when the last ATTEMPT was made.
	if r.Nudge("test") {
		t.Error("second Nudge inside the rate-limit window = true, want false (suppressed)")
	}
	if next2, _ := r.NextNudgeAt("test"); !next2.Equal(want) {
		t.Errorf("NextNudgeAt after a SUPPRESSED nudge = %v, want unchanged %v", next2, want)
	}

	advance(2 * time.Second)
	next, ok = r.NextNudgeAt("test")
	if !ok {
		t.Fatal("ok = false after the window elapsed, want true")
	}
	if next.After(fakeNow) {
		t.Errorf("NextNudgeAt = %v, still after now (%v) once the window has elapsed", next, fakeNow)
	}
}

...
// installFailAuditTrigger mirrors identity/audited_write_test.go's helper
// of the same name exactly (that file's own doc comment explains why a
// second raw connection is required: this package has no other way to
// reach *store.Store's own unexported *sql.DB). Duplicated here rather
// than imported because it is a _test.go helper in a different package.

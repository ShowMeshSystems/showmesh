...
// installFailAuditTrigger is
// internal/coordinator/identity/audited_write_test.go's own helper,
// reproduced here for the identical reason that file gives: a real
// SQLite trigger through a second raw connection to the same on-disk
// file, so an audit-write failure is proven against the real
// store.Store.InTx / AppendAuditEntry path, never a hand-rolled fake.

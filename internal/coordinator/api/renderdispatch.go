...
func (h *handlers) executeRenderDispatch(ctx context.Context, now time.Time, in renderDispatchInput) (v1.RenderCommandResult, *v1.Problem, error) {
	...
	if in.Action == "render.transport.probe" {
		...
		confirmed, outcomeState, outcomeReason = h.confirmRenderTransportProbe(ctx, in.SurfaceID, dispatchedAt)
	} else {
...
// confirmRenderCommand uses for surface.pipeline.state.
func (h *handlers) confirmRenderTransportProbe(ctx context.Context, surfaceID string, dispatchedAt time.Time) (confirmed bool, outcomeState, outcomeReason string) {
	if h.deps.Observations == nil {
		return false, string(observation.StateNotCollected), "no observation source is configured"
	}
	absDeadline := time.Now().Add(renderCommandConfirmDeadline)
	...
	for {
		confirmed, outcomeState, outcomeReason = h.evaluateRenderTransportProbe(ctx, surfaceID, dispatchedAt)
		if confirmed {
			return true, outcomeState, outcomeReason
		}
		if !time.Now().Before(absDeadline) {
...
// for surfaceID — deliberately regardless of its VALUE. Unlike
// evaluateRenderSurfaceState, which confirms only a specific desired
// pipeline state, a probe has no desired transport value: an operator
// asking "can this node send NDI now?" is equally well answered by true
// and by false. Refusing to confirm a correctly-reported false would make
// "the runtime genuinely is not installed" indistinguishable from "the
// probe never ran," which is exactly the false-claim direction ADR-026
// decision 6 forbids.
func (h *handlers) evaluateRenderTransportProbe(ctx context.Context, surfaceID string, notBefore time.Time) (confirmed bool, outcomeState, outcomeReason string) {
	kind := observation.ResourceSurface
	...
	if len(candidates) == 0 {
		return false, string(observation.StateNotCollected), "no surface.transport.available observation is recorded for this surface yet"
	}
	resolved := ResolveObservations(candidates)
	o := resolved[0]
	src := o.Source

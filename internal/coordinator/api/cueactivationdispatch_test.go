...
// TestDispatchCueActivationsNeverTouchesTheLiveSessionOrProbesTheSchedule
// proves no command reaches [cueactivation.AudioSessionID] or
// [cueactivation.ScheduleProbeSessionID] before cue.activate confirms.
func TestDispatchCueActivationsNeverTouchesTheLiveSessionOrProbesTheSchedule(t *testing.T) {
	now := testNow
	setup := newAudioDispatchTestSetup(t, fixedClock(now))
	...
	if armed := dispatchedActionsToSession(setup, cueactivation.AudioSessionID); len(armed) != 0 {
		t.Fatalf("dispatched %d command(s) against the live show session before cue.activate, want 0 (%+v)", len(armed), armed)
	}
	...
	for _, nodeID := range nodeIDs {
		...
		setup.pub.mu.Lock()
		for _, d := range setup.pub.dispatched {
			if d.NodeID == nodeID && d.Action == "cue.activate" {
				activateCount++
			}
		}
		setup.pub.mu.Unlock()
		if activateCount != 1 {
			t.Fatalf("node %q: %d cue.activate command(s) dispatched, want 1", nodeID, activateCount)
		}
	}
}

// TestDispatchCueActivationsReplaysOnRepeatTick proves calling
// dispatchCueActivations again over the identical, unchanged activations
// map answers entirely from the replay path: no new command is published.
func TestDispatchCueActivationsReplaysOnRepeatTick(t *testing.T) {
	now := testNow
	setup := newAudioDispatchTestSetup(t, fixedClock(now))

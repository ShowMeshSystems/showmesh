...
func (h *handlers) dispatchWeatherDelayNodeCommand(ctx context.Context, now time.Time, action, kind, idempotencyKey string, plan mqttproto.WeatherDelayPlan, nodeID string, ac authContext, clientAddr string, allowHTTP bool) v1.WeatherDelayTargetOutcome {
	...
	go func() {
		defer wg.Done()
		mqttOK, mqttReason, mqttDispatchedAt, mqttAlertPlaying, mqttAlertReason = h.weatherDelayMQTTNodeCommand(ctx, now, action, kind, idempotencyKey, plan, nodeID, ac, clientAddr)
	}()
...
func (h *handlers) weatherDelayMQTTNodeCommand(ctx context.Context, now time.Time, action, kind, idempotencyKey string, plan mqttproto.WeatherDelayPlan, nodeID string, ac authContext, clientAddr string) (ok bool, reason string, dispatchedAt *string, alertPlaying bool, alertReason string) {

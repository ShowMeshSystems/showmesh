...
const (
	...
	// AudioNodeRoleDefault is the role a payload that omits "role" takes
	// when it DOES declare an LTC route — ADR-045 is additive, so a
	// pre-ADR-045 audio.node object (there was only ever one per
	// installation, and it always carried both program and LTC) decodes to
	// the role that describes what it already was, rather than requiring
	// every existing installation to be re-written the day this ships. A
	// payload that omits "role" AND declares no LTC route takes
	// [AudioNodeRoleProgram] instead: see [defaultAudioNodeRole].
	AudioNodeRoleDefault = AudioNodeRoleProgramLTC
)

// defaultAudioNodeRole is the role a payload that omits "role" takes.
// A program-only declaration (no ltcRoute, the shape a two-output
// interface must use) cannot be this installation's LTC emitter, so it
// must not default into the one role [ValidateAudioNodeRoleUniqueness]
// allows only one node to hold; two program-only nodes would otherwise
// refuse each other over LTC neither of them emits.
func defaultAudioNodeRole(ltcRoute string) string {
	if ltcRoute == "" {
		return AudioNodeRoleProgram
	}
	return AudioNodeRoleDefault
}
...
func DecodeAudioNodePayload(raw string) (AudioNodePayload, *ValidationError) {
	...
	role, verr := decodeDefaultedEnum(top, "role", "role", defaultAudioNodeRole(ltcRoute), audioNodeRoles)
	if verr != nil {
		return AudioNodePayload{}, verr
	}
	if role == AudioNodeRoleProgramLTC && ltcRoute == "" {
		return AudioNodePayload{}, &ValidationError{
			Code: ValidationCodeFieldInvalid, Field: "role",
			Detail: fmt.Sprintf("role %q requires an ltcRoute and ltcChannel: a node that declares no LTC route emits no LTC and cannot be the installation's LTC emitter", AudioNodeRoleProgramLTC),
		}
	}

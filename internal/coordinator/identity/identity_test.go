...
func TestRoleScopes(t *testing.T) {
	cases := []struct {
		...
		{RoleOperator, []Scope{ScopeNodeRead, ScopeFPPRead, ScopeObservationRead, ScopeEventRead, ScopeShowMacroRun, ScopeDevicePower, ScopeFPPCommand, ScopeResolumeAction, ScopeRenderCommand, ScopeShowActionInvoke, ScopeAudioCommand, ScopeNightCommand, ScopeShowEmergencyStopInvoke, ScopeCueActivate}},
...
func TestRoleHas(t *testing.T) {
	...
	if RoleScheduler.Has(ScopeNodeRead) {
		t.Errorf("RoleScheduler.Has(node:read) = true, want false: scheduler holds only show:macro:run, night:command, fpp:observe, and fpp:fallback")
	}

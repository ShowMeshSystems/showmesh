...
The node's installer sends the code to `POST /api/v1/node-enrollments/redeem` and writes what comes back into `/etc/showmesh/agent.env`. On the built-in broker the coordinator writes the node's entry into `mosquitto/passwd` and rebuilds `mosquitto/acl.generated.conf` by the same rules as `generate-credentials.sh`. Both scripts keep working, and either may run after the other.
...
What replaced `SHOWMESH_API_TOKEN` is not another shared secret. [ADR-024](../docs/decisions/ADR-024-identity-authorization-and-audit.md) gives this coordinator real principals with roles and audit attribution, in two credential forms, a browser session and an API token. Roles are named scope bundles, which satisfies ARCHITECTURE section 10.4 for **which action** a principal may perform and explicitly **not** for which target: any principal holding a scope holds it everywhere. **The show VLAN remains the actual security boundary**, and ShowMesh terminates no TLS ([ADR-022](../docs/decisions/ADR-022-operator-ui-serves-the-api-same-origin.md)), so the session cookie is readable on the wire without a TLS proxy in front.

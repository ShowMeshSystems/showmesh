...
| **Weather delay** | The coordinator and node side of [ADR-053](docs/decisions/ADR-053-weather-delay.md) work end to end against a real broker and a real node agent, proven with an HTTP stand-in for an FPP player and its plugin weather gate, not a real one. The FPP plugin's own output gate, which is what makes a real player's channels go to zero, is built in a separate repository and has not merged there, so no installed plugin currently backs the coordinator's hold. Nothing here has run on real hardware, a deployed fleet, or against a live weather service. |
...
**Stack:** Go 1.26 (coordinator, agent, CLI), TypeScript/React/Vite (Operator UI), MQTT/Mosquitto (control plane), SQLite via `modernc.org/sqlite` (storage: pure Go, because the coordinator must build CGo-free for a static distroless multi-arch image), GStreamer (the render and audio engines, both shipping), Apache-2.0.

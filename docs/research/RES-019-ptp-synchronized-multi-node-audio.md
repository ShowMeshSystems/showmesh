...
### 8.1 Stage 0 hardware measurement (2026-09-11, L2)
...
The rate lock holds the two outputs to half a millisecond, and what remains is a CONSTANT ~56 ms difference between the two signal paths: not drift, and no clock work removes it. A second run at a different PipeWire graph quantum moved the offset from about 53 ms (quantum 1024) to about 81 ms (quantum 256) — the fact `configuration` exists to record and `§8.4` exists to keep visible.

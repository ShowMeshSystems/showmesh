...
```text
outputLatency: { valueUs, measuredAt, method, reference, confidence, configuration }
```

`method` is one of `unmeasured` (default; applies 0 and reports `unknown`), `loopback` (electrical: output fed to an input on a third device, cross-correlated), `acoustic` (microphone at a defined position, cross-correlated against the reference node), or `declared` (operator-entered from a datasheet, lowest confidence). The value is subtracted from that output's `T0` so the sample reaches the air at the intended instant. `configuration` records the buffer/quantum/sample-rate configuration `valueUs` was measured under, required whenever `method` is not `unmeasured` (§8.3).

### 8.1 Stage 0 hardware measurement (2026-09-11, L1)

Two rate-locked nodes (a MOTU M4 on an x86 node with a PTP hardware clock, and a Scarlett Solo on a Raspberry Pi 3B+ with none) were recorded together through one interface while both played the same click track from a shared start instant on the PTP clock, 150 clicks over 5 minutes:

```
offset between the two outputs   mean 55.997 ms   sd 0.146 ms   peak-to-peak 0.521 ms
```

The rate lock holds the two outputs to half a millisecond, and what remains is a CONSTANT ~56 ms difference between the two signal paths: not drift, and no clock work removes it. The run's PipeWire graph quantum, sample rate, and software versions were not recorded at measurement time, so this figure cannot be reconciled against the separate quantum comparison below or reproduced against a stated configuration; the second run recorded moving the offset from about 53 ms (quantum 1024) to about 81 ms (quantum 256), which is exactly why `configuration` exists to record and `§8.4` exists to keep visible.

**This is hardware evidence for why the seam is needed, not a shipped value.** No `audio.node` object in this repository carries a non-`unmeasured` `outputLatency` as of this seam (Track I I5, `claude/output-latency-offset`); the 56 ms figure is recorded here as the motivating measurement, never as a default, a fallback, or a fixture value standing in for a real per-installation measurement. Every output ships `unmeasured` until an operator runs §8.2 against that specific node's own hardware.

### 8.2 Measurement procedure

**Rig.** Both nodes play a click track (a 20 ms 1 kHz burst every 2 s) from a shared scheduled start instant on the media clock (§6). One node (or a third capture device for `acoustic`) records the outputs and a small analyzer finds each burst's onset and reports the per-burst offset between paths. This is the same shape the §8.1 measurement used and the one this record expects `showmeshctl`/API tooling to eventually wrap, not a different procedure invented for real installations.

**`loopback`.** Feed each output electrically into a separate input channel of one audio interface (a cable per output, not a mic). Run the click track for several minutes (the §8.1 run used 150 clicks over 5 minutes) and take the analyzer's mean burst-to-burst offset between the reference path and the path under test as `valueUs`, the run's standard deviation as supporting evidence for `confidence`, and the interface used as `reference`.

**`acoustic`.** Same click track, a calibrated microphone at a defined listening position instead of an electrical loopback. Acoustic measurement additionally carries the speed-of-sound delay from each speaker to the microphone, so `reference` must name the microphone position (and, if it matters for the installation, the per-speaker distance correction applied before recording `valueUs`); this method's `confidence` should read lower than an equivalent `loopback` run for that reason.

**`declared`.** No measurement: an operator transcribes a datasheet or vendor-stated figure into `valueUs` and cites the datasheet section in `reference`. Lowest confidence of the three real methods, and the only one where `configuration` may legitimately read "not applicable" rather than naming a buffer/quantum/rate.

**Re-measurement is not optional.** A value that moves more than a few hundred microseconds between engine restarts is not static; RES-019's own finding (§8.1) is that the offset moves with PipeWire's graph quantum, and this repository has no way to guarantee a node's buffer configuration is unchanged across a restart. Re-run §8.2 at every engine start where the configuration might have changed, and always after a driver, buffer, sample-rate, or PipeWire configuration change: never assume a prior measurement still holds.

### 8.3 What must be recorded together

A `valueUs` with no attached provenance is a guess wearing a number's clothes. `outputLatency`'s decode (`internal/coordinator/config/audionode.go`) enforces this structurally: `method: unmeasured` (the default) may carry no other field at all, and every other method requires `valueUs`, `measuredAt`, `reference`, `confidence`, and `configuration` together: a measurement missing any one of those is refused, not silently accepted with an empty field.

### 8.4 Why staleness is visible rather than auto-detected

§8.1's quantum finding means a value is only valid for the configuration it was measured under, but no engine or sample-path code in this repository observes PipeWire's live graph quantum (out of scope for the seam that introduced `outputLatency`: no engine changes, no sample-path code). Rather than pretend to detect staleness automatically, `configuration` is mandatory and always shown next to `valueUs` (API, `showmeshctl`, and the node page): a configuration change is visible to whoever reads the value, and the re-measurement rule in §8.2 is the operator's own trigger to act on it. A later seam that adds live PipeWire introspection can tighten this into an automatic check; this one does not claim to.

...
**That call crashes Arena 7.23.2 on the development laptop.** Seven `SIGSEGV`s with byte-identical faulting frames, one reproduced from `curl` with no ShowMesh process running, which is what establishes it as a fact about Arena rather than a defect in the adapter.

**Qualified 2026-08-14 by the owner, after this ADR was accepted, and the qualification is load-bearing for anyone re-reading this record.** Every crash cited here occurred on the development laptop, which is **not** the playout machine: Arena is not properly supported on that hardware and OS, and the same composition ran on the real playout machine for over a month, untouched, without incident. The crash is therefore **not** established as a property of Arena 7.23.2 in general, and the show host is **not** known to be subject to it. The capture's §14 preamble records the full correction.

**This does not weaken the decision, because the decision never depended on the crash.** Decisions 1 through 8 rest on facts about the API and the file that hold on any host: there are no collection endpoints, `/composition` is a 2.26 MB document, the file carries all three decks plus canvas size plus `versionInfo` where REST carries none of them, and `by-id` is deck-dependent for clips. Reading 2.26 MB off a playout machine on a timer to learn ids that are sitting in a file is the wrong design whether or not it segfaults. It is neither a request count nor an elapsed time: 2 reads, 9 reads and 2,046 reads all ended the same way. Controls that did not crash it were 7 minutes idle, 30 `/product` polls, and a WebSocket held open for 5 minutes.
...
**This does not make Resolume safe, and the measurement says so.** After the composition read was removed, two coordinator processes ran against Arena with zero composition reads and **Arena still crashed**, with the same faulting signature, while the only ShowMesh traffic was `/product` polling and one held WebSocket. So `GET /composition` is *sufficient* to crash Arena and removing it is *not sufficient* to prevent a crash. The capture's §14.4 records the timeline.

**Owner decision, 2026-08-14: neither the vendor report nor the show-length crash-count run is happening, and both are struck rather than deferred.** This paragraph previously called the vendor report "load-bearing rather than a formality" and the show-length run "a bench that nothing should be assumed past". Both were wrong in the same way, and it is worth naming the error because it is a general one.

The vendor report is struck because **the owner cannot act on its answer.** Arena 7.23.2 is a subscription-expired build; upgrading requires renewing, and the owner does not renew until late November — after the Halloween show and after the Christmas build starts. A report against a version he cannot leave buys a fix he cannot install. The crash-report files remain in the record if the position changes.

The show-length run is struck because **it is a measurement with no decision attached to it.** Whether Arena crashes twice an evening or ten times, the build response is identical, because it is the only response available: ShowMesh cannot patch Resolume. Recording a bench as "needed" is not the same as it being needed, and this document did that.

**What replaces both is a build item**, sequenced after D-3 because it depends on `launchClip`: ShowMesh notices Arena is gone, alerts, and — once Arena is back, however it got back — verifies the composition by decision 5's check and restores the layers that were playing. Arena comes back with nothing playing (capture §10.2), so today a crash is a dark wall until someone walks to the render host and relaunches the clips by hand. **This does not prevent the crash and nothing here should be read as though it does.** It converts an indefinite outage into a bounded one, which is the only thing in ShowMesh's power.

**Nothing restarts the Arena process — not ShowMesh, and not a watchdog on the render host.** This was proposed on 2026-08-14 and rejected by the owner the same day, and the reasoning generalises past Resolume. A process supervisor on the render host is a new daemon on the most fragile machine in the show whose failure mode is **relaunching Arena at a moment a human deliberately had it stopped** — during shutdown, during a crash loop, during a fix. It trades a way to break a working Arena for a few seconds of recovery time. The constraint that ShowMesh must stay out of process management on a machine it does not own an agent on is real; the conclusion to draw from it is that *nothing* automates that restart, not that the automation moves somewhere less visible.

So the split is: **the operator owns the process, ShowMesh owns the show state.** Relaunching Arena is a human action. Restoring what was on the wall is the part that requires knowing what the show was, which is the part ShowMesh is uniquely able to do and the operator cannot do quickly under pressure.

Whether the restore itself fires automatically on seeing Arena return, or waits for the operator, is deliberately left to the seam that builds it — with the default being **operator-triggered**, per the standing rule that this project has no reconciler silently closing desired and observed state.

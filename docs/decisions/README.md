...
| ADR-041 | Operator recovery is not a calendar intent | **Reserved 2026-08-19** for Track F, drafted and awaiting the owner's acceptance. Narrows ADR-038 decision 2's "closed command vocabulary" to mean the set of lifecycle intents FPP's calendar invokes, and places `end-session` alongside that set as an operator recovery action rather than inside it. The substance was approved by the owner on Linear SM-82; what is outstanding is accepting the record. |

...
| [ADR-045](ADR-045-multi-node-audio-and-roles.md) | An installation may declare more than one audio node, with roles | Accepted (owner, 2026-08-26; transcribes SM-308's five frozen decisions: optional per-output target node on show.cue, one LTC emitter enforced at authoring and readiness, audio.node role/zone, one installation-wide audio.settings this season, and a list of target nodes for night-mode bed/announcements. Extends, narrows nothing; cross-node sample sync is SM-309's question.) |

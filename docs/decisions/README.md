...
| ADR-040 | The renderer extracts channels; GStreamer owns every frame the audience sees | **Proposed** (drafted 2026-08-17 by the Track B session for the owner to rule on; interprets ADR-007's per-frame boundary. The number is reserved here so a parallel branch cannot take it; the record itself lives on `track-b/integration` until it is ruled on.) |

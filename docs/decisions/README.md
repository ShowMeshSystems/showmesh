...
| [ADR-017](ADR-017-showmesh-owns-audience-audio.md) | ShowMesh owns audience audio; nodes play local media | Accepted (narrowed by ADR-046 and ADR-047, 2026-08-28: rate trim against a locked shared clock is permitted, slews, seeks, and position-feed chasing are not; FPP's AES67 stream may be the primary program source with local playback as standby) |
...
| [ADR-045](ADR-045-multi-node-audio-and-roles.md) | An installation may declare more than one audio node, with roles | Accepted (owner, 2026-08-26; transcribes SM-308's five frozen decisions: optional per-output target node on show.cue, one LTC emitter enforced at authoring and readiness, audio.node role/zone, one installation-wide audio.settings this season, and a list of target nodes for night-mode bed/announcements. Extends, narrows nothing; cross-node sample sync is SM-309's question. Decision 1 **superseded** for audio and announcement outputs by ADR-049.) |
...
| [ADR-047](ADR-047-fpp-aes67-is-the-primary-program-source.md) | FPP's AES67 stream is the primary program source; local playback is the standby | Accepted (owner, 2026-08-28; **narrows** ADR-017's local-playback and unused-FPP-output decisions. One program bus, ordered sources, ShowMesh-owned sources always local, output backends unchanged in kind. Carries an ordering rule: clock provider and scheduled start land before AES67 receive. Built by Track I on `dev/clock-sync`; gated on the FPP output-group `secondsElapsed` capture.) |
| [ADR-048](ADR-048-signed-fpp-fallback-program.md) | A signed FPP fallback program preserves a running show | Accepted (owner, 2026-08-30; FPP executes only a pre-authorized entry-key-to-Cue mapping while the coordinator is unavailable. It does not become a scheduler or a general command authority. Recovery waits for the next normal scheduled-show boundary.) |
| [ADR-049](ADR-049-same-audio-on-several-nodes-at-one-instant.md) | A Cue's audio plays on several nodes at one instant | Accepted (owner, 2026-09-15; **supersedes** ADR-045 decision 1 for audio and announcement outputs: a list of target nodes, one start instant chosen from the `program+ltc` node's clock, unaligned start on arrival when no locked clock exists. LTC stays on one node. Amended 2026-09-16 with decisions 6 to 9: the start instant is read once per activation with a wait that covers a real prepare, a night bed lists its target nodes and every listed node plays all of it, a multi-node bed starts, changes items, and resumes together, and a multi-node night announcement has its file on every node and starts together. Decisions 1 to 5 built; decisions 6 to 9 not yet built.) |
| [ADR-050](ADR-050-operator-override-of-a-cue-catalog-claim-conflict.md) | An operator may override a cue catalog's exclusive-claim conflict | Accepted (owner, 2026-09-16; a deploy `override` bypasses only the H0.5 exclusive-claim conflict, recorded per node and scoped to the exact revision and accepting operator, never carried forward. Readiness reports an overridden conflict as a warning, not a failure. Auto-deploy never overrides. Does not close H0.5's conservative cross-playlist concurrency test.) |

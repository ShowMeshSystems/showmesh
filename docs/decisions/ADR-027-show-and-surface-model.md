...
- Named-model mapping depends on xLights metadata reaching ShowMesh, which needs the FPP Connect compatibility work. Until that lands, **manual configuration is the only path that exists**, so treating it as a fallback would mean shipping day-0 on a path the design considers second-class.
  ...
  **This correction was itself narrowed twice on the same day, and the sequence is worth keeping.** First it was widened: the Controllers tab's "Upload Output" was raised as a second surface that genuinely pushes configuration, so both directions appeared to exist. Then [RES-003](../research/RES-003-xlights-fpp-connect-compatibility.md) §9.7b traced it and found that **"Upload Output" pushes per-port pixel, panel, serial and PWM wiring configuration and never touches model definitions at all.** Models reach a device only through FPP Connect's own Models dropdown, which defaults to "None".

  So the settled position is: **the FSEQ channel set is pulled from the target; model definitions are pushed, but through FPP Connect rather than the Controllers tab.** Both directions are real and both live on the same surface.

  xLights supplies the strongest evidence that a render node does not belong on the Controllers-tab surface at all: it already ships variants named "FPP Player Only" and "FPP Video Playing Remote Only" with zero pixel ports and **no upload support declared**, which is xLights' own model of a device that plays media and drives no outputs.

  **Decision 4's conclusion has now survived three corrections to its reasoning without changing.** That is recorded rather than tidied because a conclusion outliving its justifications repeatedly is either unusually robust or insufficiently examined, and a later reader should get to judge which.

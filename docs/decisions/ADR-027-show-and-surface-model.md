...
- Named-model mapping depends on xLights metadata reaching ShowMesh, which needs the FPP Connect compatibility work. Until that lands, **manual configuration is the only path that exists**, so treating it as a fallback would mean shipping day-0 on a path the design considers second-class.
  ...
  **The correction above was itself too broad, narrowed the same day.** It said the direction of travel is the reverse of what this decision assumed, and that is true **only of the FPP Connect path** the research was scoped to. xLights has a second, separate surface: the **Controllers tab's "Upload Outputs"**, which genuinely does **push** configuration to a controller and which the operator recalls as the mechanism by which model definitions reach a device. So both directions exist, on different surfaces, and neither replaces the other. The FSEQ channel set is pulled; model definitions are pushed. That second path is being researched into RES-003 and is the one that would let an operator pick a surface by model name instead of transcribing a start channel. Decision 4's conclusion has now survived two corrections to its reasoning without changing.

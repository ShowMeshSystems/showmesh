...
**Amended 2026-09-01: media type joins identity.** The identity above never named media type, so an FSEQ and its own audio, uploaded under the same show/sequence/target, held the SAME identity slot: the second upload superseded the first regardless of what kind of file either one was, and a later render assignment could resolve the sequence to the wrong file (an operator's asset upload for a Halloween number's audio track silently displacing that same number's FSEQ, discovered only when a node refused the resulting mismatched file with a bad-magic error naming neither the upload nor the supersession that caused it).

**Identity is now show plus logical sequence plus target plus media type plus content hash.** An FSEQ and an audio asset for one sequence are different identities and may both be current at once; uploading one never supersedes the other. Schema v28 widens the `assets_current` structural constraint (decision 1's own machinery, `internal/coordinator/store/migrations.go`) to key on media type as well, and every store-layer lookup that resolves "the current asset for this tuple" (`assetsync/cuecatalog.go`'s `resolveAssetFor`, `assetsync/showmeshaudio.go`'s `resolveAssetForWithSize`, and `store/assets.go`'s supersession and identity-match queries) takes the intended media type as an explicit parameter rather than inferring it from sort order. A caller with no media type of its own to state (a media-playlist item whose own type implies audio, for instance) supplies the constant it already knows; nothing stores the media type as data an operator must name.

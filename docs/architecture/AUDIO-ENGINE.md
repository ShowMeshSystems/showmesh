...
The Audio Engine consumes playlists; it does not own a playlist authoring store. Track H supplies an immutable, resolved `show.playlist` revision whose entries reference same-show Cues, plus repeat policy, resume/restart policy, requested item transition, and exact resolved audio asset ids and content hashes. A Cue identifies the intended show content without making a mutable filename the authoring identity. Resting Mode references and pins that Playlist revision from its Night Session configuration rather than embedding another ordered list. Later configuration edits or asset supersession do not alter the active Playlist.
...
A pipeline crash or freeze, decode failure, media disappearance, media present but no longer matching its pinned content hash, sample-rate or route change, timing-authority loss, and a device reappearing after loss are distinct faults with distinct observations. Media disappearing and media mismatching are deliberately not the same fault: the first sends an operator to restore an absent file, the second to a file that is present but wrong, and collapsing them sent operators looking for the wrong thing. None may be collapsed into `stopped` or silently restart into an unknown position. A reappearing device remains unavailable until its route, channel separation, clock relationship, and current asset are revalidated; resumption is an explicit operator action during the initial release.
...
1. **Identify the failed component.** Read `audio_session.fault.kind` for the affected sessions and `node.audio.device.state`, `node.audio.program.state` and `node.audio.ltc.state` for the node. The fault kind names what broke: `pipeline_crash` or `freeze` is the engine, `decode_failure`, `media_disappeared` or `media_mismatch` is the asset (missing versus present-with-wrong-content, respectively), `route_changed` is the physical output, `timing_authority_lost` is the clock relationship.

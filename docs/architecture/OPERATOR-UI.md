...
The decision this section demanded is now made, for both surfaces, in [ADR-024](../decisions/ADR-024-identity-authorization-and-audit.md). It supersedes ADR-021's optional shared secret and ADR-022 decision 4's `sessionStorage` token, and it lifts the bar on the first write endpoint. Writes always require an authenticated principal, with no opt-out. Reads keep the previous posture, open by default and closable by configuration, because a credential problem must never cost the operator visibility of the show.

Client obligations that follow, and that this document owns:

- **The browser holds a coordinator-minted HttpOnly session cookie**, not a token in storage. The UI container forwards it and never mints or validates one, per ADR-022 decision 2. A session slides on any request carrying the cookie, including reads, and expires only after 90 consecutive days without use.
- **Being signed out is a persistent state, never a modal at the moment of use**, and it covers three cases rather than one: an invalidated session, an expiring session, and a device that has never authenticated at all. The third is a new phone, a cleared cookie, a private tab, or an installed progressive web app with its own cookie jar, and without it the first authentication demand still arrives when the operator presses a button.
- **The bearer-paste field survives as break-glass**, so a machine token can act from a phone when the session path is broken and host access is not available.
- **A scope list carries freshness and renders as `unknown` when stale, never as permitted.** This is §2.4 applied to authorization: a control enabled because the client last heard it was allowed is the same defect as evidence rendered as current because it was current once.
- **A closed change stream may mean a revoked session.** The coordinator closes the connection rather than announcing revocation as a new event kind, so the client's existing reconnect and snapshot path surfaces the `401` as an explicit authentication state.

Role-based access is now real rather than future: `viewer`, `operator`, `admin`, and a machine `scheduler` role, expressed as scope bundles. Authorization decisions are expressed server-side and returned to the client rather than inferred by it, and a control the principal may not use is rendered disabled with a stated reason rather than hidden, per §11.

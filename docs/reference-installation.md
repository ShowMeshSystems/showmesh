...
| Unaccounted FPP (`FPP-01`) | unknown | unknown | FPP **9.2** (branch v9.2) | **Not part of the topology above, and its role is an open question for the operator.** It does not appear in the address list and was not probed over REST. It is known only because it publishes to the MQTT broker below, where **every one of its topics arrived retained and it published nothing live during a 60-second capture on 2026-08-11**. Its `port_status` is `[]`. Whether it is powered off, decommissioned, or the standby K16-Pro is unresolved |

**Per-board port element counts, confirmed live 2026-08-11.** `/api/fppd/ports` returns a different number of elements per board and the element set is heterogeneous, so neither the count nor the layout may be hard-coded: FPP-Main returns `[]` (a Pi with no output cape — zero ports is a true fact, not a failure), FPP-remote-01 returns 32 elements (16 carrying `ma`, 16 smart-receiver positions), FPP-remote-04 returns 48 (16 carrying `ma`, 32 smart-receiver positions). `bank` labels are non-contiguous and differ per board (`Ports 1-4`, `Ports 13-16`, `Ports 17-20`, `Ports 21-24`). `pixelCount` was absent on every element of every board.

**Player and remote report structurally different status documents.** On FPP-Main (player mode) `/api/fppd/status` carries `current_playlist`, `next_playlist`, `scheduler` and `repeat_mode`. On both remotes those four keys are entirely absent and are replaced by `playlist`, `sequence_filename`, `media_filename` and `seconds_elapsed`. FPP-remote-04 additionally omits `warnings` and `warningInfo` while its MQTT `warnings` topic publishes `[]`, so absence over REST and an empty list over MQTT describe the same fact by different means.
...
|---|---|---|
| 2026-08-11 | Live read-only probe for Step 5. Recorded the MQTT section above; confirmed per-board port element counts and the empty array on FPP-Main; recorded that player and remote report structurally different status documents; found a fourth FPP (`FPP-01`, v9.2) present only as retained MQTT state | Nothing invalidated. RES-011 gains shape evidence only — every `ma` reads 0 with the display de-energized, which is not evidence that current telemetry works |

...
Status: D-1 built and verified against the running Arena 2026-08-14; D-2 built 2026-08-14 and closed 2026-08-15; **D-3 built, reviewed and closed 2026-08-15, and driven against a real Arena the same day** ([results](../bench/TRACK-D-LIVE-VERIFICATION.md) §6, on the development laptop and not the playout machine). **Seam B ([ADR-037](../decisions/ADR-037-resolume-references-are-names-not-ids.md) decisions 1 through 6), seam E (Resolume as a first-class observability resource), seam C (a macro step can drive Resolume), [D-3a, crash recovery](TRACK-D-D3A-CRASH-RECOVERY-SPEC.md), and D-4 (the Operator UI) are all built, reviewed and merged, all 2026-08-16 — the fifth and last seam this track owed.** All five are on the integration branch `track-d/integration` with `make check`, `go test -race` (27 packages, 0 failures) and `make test-integration` (236s) green on the combined tree; **none of the five is merged to `main`, and none has been driven end to end in a browser against a real Arena** — the live-Arena evidence in this document is D-1/D-2/D-3's from 2026-08-15 plus isolated API-level checks captured during the D-4 build, and none of it covers all five seams together as merged. D-4's real-browser requirement found a bootstrap regression in D-3a's own code (the built-in recovery principal made `Store.HasAnyPrincipal` permanently true, so no fresh deployment could ever be claimed), fixed separately. D0 (the timecode bench) is outstanding and not gating, but **it now carries a case D-3a depends on**, see the deliverables section. Specified 2026-08-13. **Day-0 scope**, promoted from "not sequenced" the same day. **The next action is the owner sequencing this branch against Track E's PR #3 and the uncommitted Track F work in the main checkout**, not further building.
...
>
> **D-2 and D-3 have now run against a real Arena**, thirteen checks passed and none failed, on the development laptop rather than the playout machine. [The results are in the checklist](../bench/TRACK-D-LIVE-VERIFICATION.md) §6, along with what is still not run.
...
**One D0 case became load-bearing on 2026-08-16, and it is the Resolume-restart one.** [D-3a](TRACK-D-D3A-CRASH-RECOVERY-SPEC.md) §10.1 scopes crash recovery to the layers ShowMesh drove explicitly, on the owner's reasoning that a timecode-driven timeline resumes on its own once LTC keeps rolling through the restart. **Nothing has tested that.** If D0 finds the timeline does not resume, §10.1 is the decision that reopens, and its two alternatives are a low-rate layer read during a show or restoring layer state rather than clip state. That makes this bench the thing standing between "crash recovery covers the wall" and "crash recovery covers the half of the wall ShowMesh launched", which is a materially different promise to make to an operator.
...
- ~~**A composition with more than one page surfaces as visible evidence.**~~ **Replaced 2026-08-14**: the deck race is removed by dropping positional addressing, and the surviving criterion is that **an action records whether the selected deck changed between its decision and its confirmation.** **Closed 2026-08-16 by D-3a**: `ActionOutcome.SelectedDeckChanged` is set for a confirmed `launchClip` via `deckChangedSinceDecision`, per [TRACK-D-ADAPTER-SPEC.md](TRACK-D-ADAPTER-SPEC.md) §3.8, reassigned to D-3a from seam C during the build. Built and gated green on `track-d/integration`; not yet merged to `main` and not yet measured against a real deck change on a real Arena.
- A macro step launches a clip, activates a layer, and triggers a column, each confirmed by Resolume reporting the result rather than by the message being sent. **"Activates a layer" resolves to bypass and master**, because the capture established there is no `active` field on a layer. **Closed 2026-08-16 by seam C**: `show.action.target.integration` accepts `resolume`, carrying one of D-3's seven actions and seam B's named-reference vocabulary, dispatched through the identical `api.ResolumeActionDispatcher` the HTTP handler uses so confirmation, deadlines and the identity gate apply unchanged. Built and gated green on `track-d/integration`; not yet merged to `main` and not yet run against a real Arena from a macro.
...
- Resolume state appears in the Operator UI with provenance and freshness. **Closed 2026-08-16 by D-4**: a Resolume dashboard panel and a `/resolume` view render observations through the same evidence component the rest of the UI uses, plus the composition inventory, the ambiguous-clip list and the crash-recovery record. **Qualifier that must travel with this closure: it has not been driven end to end in a browser against a real Arena.** An earlier build commit (`b21e9bb`) did drive a real dispatch and a real ambiguous-clip disambiguation against the operator's Arena 7.23.2 in an actual browser, and the review-fix pass captured a genuine refusal string from a live launchClip dispatch against a real Arena to prove its sanitizer — both are real API-level evidence against the real Arena. But the `claude-in-chrome` extension would not connect during the review-fix pass, so the three manual browser checks that review specified, against the code as merged, were never completed. Built and gated green on `track-d/integration`; not yet merged to `main`.
- Timecode loss produces the defined, operator-visible response decided above. **Still outstanding; this is D0.**
- A Resolume restart mid-show recovers to a defined composition state rather than an undefined one. **Partially closed 2026-08-16 by D-3a**, and the partiality is load-bearing, not incidental: the restore covers only the layers ShowMesh drove explicitly through a confirmed action or a survey (countdowns, resting visuals, pre-show text, blackout), because path one's timeline clips are launched by LTC and appear in neither of D-3a's two evidence sources. Whether a timecode-driven timeline resumes on its own through a restart, which the owner's working assumption says it does, **is untested and is now a D0 case** (see the deliverables section); if D0 finds otherwise, D-3a §10.1 is the decision that reopens. D-3a is built and gated green on `track-d/integration`, not yet merged to `main`, and not yet driven through a real Arena restart.

...
Status: D-1 built and verified against the running Arena 2026-08-14; D-2 built 2026-08-14 and closed 2026-08-15; **D-3 built, reviewed and closed 2026-08-15, and driven against a real Arena the same day** ([results](../bench/TRACK-D-LIVE-VERIFICATION.md) §6, on the development laptop and not the playout machine). **Seam B ([ADR-037](../decisions/ADR-037-resolume-references-are-names-not-ids.md) decisions 1 through 6), seam E (Resolume as a first-class observability resource) and seam C (a macro step can drive Resolume) are built, and [D-3a, crash recovery](TRACK-D-D3A-CRASH-RECOVERY-SPEC.md) is built, all 2026-08-16.** All four are merged to the integration branch `track-d/integration` with `make check`, `go test -race` and `make test-integration` green on the combined tree; **none of the four is merged to `main`, and none has been driven against a real Arena** — the live-Arena evidence in this document is D-1/D-2/D-3's from 2026-08-15 and does not cover anything built 2026-08-16. **The next build item is D-4, the Operator UI**, per the same build order (owner, 2026-08-16: ADR-037 seam B, then D-3a, then D-4) — **D-4 is building on its own branch and is not part of this status.** D0 (the timecode bench) is outstanding and not gating, but **it now carries a case D-3a depends on**, see the deliverables section. Specified 2026-08-13. **Day-0 scope**, promoted from "not sequenced" the same day.
...
- Resolume state appears in the Operator UI with provenance and freshness. **Still outstanding.** Seam E (2026-08-16) puts Resolume observations on the API, the change stream and `showmeshctl`; the Operator UI surface itself is D-4, building and not part of this status.

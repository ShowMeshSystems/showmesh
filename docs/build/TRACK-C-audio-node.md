...
- **Decision state**: SM-70, SM-78, and SM-83 are resolved. SM-71 and SM-72 remain open. SM-84 remains open for reconciliation because its package comment names a separate `showmesh-ltcgen`, while SM-69 records the owner-updated linked-libltc/cgo design with no generator process.
...
## C5, INCOMPLETE 2026-08-18

`f7743c5` contains a dormant supervised-process design and structural graph tests. It does not contain the owner-selected production shape. SM-69 now requires libltc linked into the native agent through cgo, feeding the real go-gst pipeline selected in SM-68, with no separate `showmesh-ltcgen` process.

- **LTC is generated live through libltc, never played from a pre-rendered file.** The current owner ruling links libltc into the agent through cgo so Go forwards C-generated PCM into the go-gst graph. The earlier supervised external-process design is superseded and must be removed under SM-69.
...
- **The abandoned process supervisor observes liveness through heartbeats rather than OS process existence.** That behavior is historical scaffolding, not a production requirement: SM-69 replaces the process shape with linked libltc and must define truthful liveness from the real engine path.
- **The structural graph test places stereo program on channels 1–2 and LTC on channel 3.** SM-78 resolved that as the default, not the only layout: `audio.node` must carry explicit, non-overlapping program and LTC indices, including mono program on channel 1 with LTC on channel 2. Physical discreteness stays a C0b commissioning check.
...
Still no audio engine: production never consumes generator output, and nothing here has produced timecode on a wire.
...
- The LTC package record must be reconciled: SM-69 selects libltc linked into the agent with no generator process, while SM-84's later package comment names a shipped `showmesh-ltcgen`.

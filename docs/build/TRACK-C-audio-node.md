...
Status: **C0a-1, C0a-2, C1a, C1b, C2, and C3 complete, 2026-08-18.** Specified 2026-08-13; dependency-complete review 2026-08-17. Pipeline/graph behaviour measured in C0a-1, C1a's real-device probe test, and C2's discoverer-versus-decode measurement is L2 (see RES-007); everything hardware- or device-dependent, and every engine seam past C3 (C4 onward), remains L0 design intent until the benches or seams below run for real. **C3 ships no audio engine**: it is the session layer built against a fake, and stays that way until the pipeline-control decision (Linear SM-68) is made. See "C3, complete 2026-08-18" below for exactly what that does and does not mean.
...
- **This stays true until the pipeline-control decision is made.** C4 (program mix and Resting Mode primitives) and every later seam that needs a real pipeline stay blocked on Linear SM-68, not on anything in this track's own code.
...
- **`audio.settings`'s four fields have no consumer.** Drift-ignore threshold, default fade curve, default fade duration, and default background gain ceiling are stored, revisioned, and audited, but nothing in this codebase reads them — no session engine exists yet to apply them. They wait on C3/C4.

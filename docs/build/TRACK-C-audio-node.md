...
Status: **IN REVIEW.** As of 2026-08-20 (merges `7f3b102`, `2396351`, `0b5593f`, `f128049`, `49c95bb`), `main` carries explicit `audio.node` program/LTC channel indices, a broker-loss integration test that runs against its own isolated Mosquitto rather than being skipped, a real GStreamer audio engine package (`internal/agent/audio/gstengine`, built behind a `cgo` build tag), three reliability fixes to the session/persistence layer, and a fix for a session on a dead output pipeline that kept reporting its last-known state. See the dated section below. **The agent still constructs `audio.NewFakeEngine`; the real engine is not yet wired in. Nothing has produced sound.** Production LTC generation (SM-69), the wiring seam (SM-68), and assembled-path acceptance remain open.
...
| Operational software | No | Production still wires an unavailable fake engine. A real GStreamer engine package exists on `main` as of 2026-08-20 but `internal/agent/agent.go` does not yet construct it; C5 is not a production LTC path. |

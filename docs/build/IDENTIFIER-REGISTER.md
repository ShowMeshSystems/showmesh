...
| v28 | shipped | widens the `assets_current` partial unique index to key on `media_type` as well as show/sequence/target, so an FSEQ and an audio asset may both be current for one sequence at once instead of the second upload superseding the first (ADR-028 decision 1's amendment). A pure widening: every pre-v28 row already holds exactly one media type per tuple, so no data fix runs |
| v29+ | unallocated | free |

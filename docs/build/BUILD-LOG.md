...
**Step 6 is complete.** [ADR-024](../decisions/ADR-024-identity-authorization-and-audit.md) was written on 2026-08-11 and implemented on 2026-08-12 by five parallel seams, then attacked by three reviews and repaired. It supersedes ADR-021 and ADR-022 decision 4. **The next action is the first write endpoint**, which Step 6 deliberately does not provide: it builds the mechanism that permits one. Two obligations transfer to that step and are recorded in BUILD-PLAN rather than left in ADR prose: decision 7's fallback trigger, and decision 11's same-transaction audit rule. Other work available in parallel: RES-008's configuration model, and `pkg/pjlink` as a protocol library at L1.

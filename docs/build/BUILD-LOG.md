...
**The next action is Step 7**, which carries **three writes rather than one**: a primitive FPP lifecycle command, the configuration write surface, and node discovery. No macro. Two Step 6 obligations must close before any of them: `identity.Service` gaining an atomic audit variant, and a deliberate decision on login CSRF.

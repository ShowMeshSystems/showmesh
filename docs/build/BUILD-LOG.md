...
**The identity and authorization ADR now exists.** [ADR-024](../decisions/ADR-024-identity-authorization-and-audit.md) was written and reviewed on 2026-08-11 as a design session with no implementation, and it supersedes ADR-021 and ADR-022 decision 4. **The next action is Step 6, which implements it.** Step 6 deliberately adds no write endpoint of its own: it builds the mechanism that permits one, and the first actual write operation is the step after. Other work available in parallel: RES-008's configuration model, and `pkg/pjlink` as a protocol library at L1.

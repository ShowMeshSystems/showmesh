...
**CI was red at HEAD and nothing but the review caught it.** Two integration tests still set `SHOWMESH_API_TOKEN`, which this step made a hard startup refusal, so the coordinator subprocess exited 1 before `/healthz` answered. The same commit had updated that file's broker plumbing and missed them.
...
**Verification gates:** `gofmt`, `go vet`, `make lint`, `CGO_ENABLED=0 go build ./...`, `go test -race -count=1 ./...` across 20 packages, `make test-integration` green against real Mosquitto with real subprocesses, UI typecheck, lint, and 287 tests over five consecutive clean runs. The login starvation attack was reproduced against the real binary before the fix (`429` on a correct password) and re-run after it (`200`). Verified by hand against the real binary: the `SHOWMESH_API_TOKEN` startup refusal, health probes answering with reads closed, the bootstrap file at `0600` with zero occurrences of the code in the log, `403` naming the missing scope, and both halves of the CSRF criterion. The UI was driven in a real Chrome against the shipped nginx image, which is where the read-loop defect was found.

...
**Status: coordinator BUILT, plugin BUILT.** Both sides now serve this. Built
is not proven: nothing here claims it has run against a real FPP host. See 2.4
for what remains.
...
**Status: coordinator BUILT, plugin BUILT,** except §3.9, which carries its own
status.
...
**Status: coordinator NOT BUILT, plugin UNVERIFIED HERE.** The coordinator
neither serves nor calls this. Everything else in section 3 is built; this
subsection is a frozen shape on the coordinator side, and no sentence in it
reports deployed coordinator behavior.

Coordinator anchor: `handleDefinitionRepublish`, which does not exist. The
plugin's half is that repository's own assertion and is not verified from here:
that repository carries files named for this feature, which is not evidence that
it serves the contract.

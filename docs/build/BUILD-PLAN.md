...
- **Audio engine and audio node are now [Track C](TRACK-C-audio-node.md)**, sequenced 2026-08-13 and no longer in this list, because day-0 scope includes audio and because the audio node is also Resolume's LTC source. The entry below is kept for its reasoning, which still holds: the architecture is decided ([ADR-017](../decisions/ADR-017-showmesh-owns-audience-audio.md), [ADR-018](../decisions/ADR-018-program-and-ltc-share-a-clock-domain.md), [ADR-019](../decisions/ADR-019-audio-device-loss-fails-silent.md), [AUDIO-ENGINE.md](../architecture/AUDIO-ENGINE.md)) and entirely unverified. It was previously unsequenced because [RES-007](../research/RES-007-audio-node-architecture.md) is critical-risk at L0 and because the multichannel interface had not been purchased. **The interface was purchased on 2026-08-13** (Behringer U-Phoria UMC204HD), which removed the second reason; the first stands, and nothing here can be raised above L0 by unit tests, since whether GStreamer holds LTC sample-aligned to program, and what drift a free-running node accumulates over a show, are bench facts. The first task remains the RES-007 prototype on the intended host and interface. ADR-018 is also a purchasing constraint, at least three output channels from one clock domain, which the purchased unit satisfies on channel count pending the output-addressing check that leads Track C.

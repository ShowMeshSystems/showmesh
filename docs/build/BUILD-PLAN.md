...
This document is that sequence. It follows CLAUDE.md's walking-skeleton build order: protocol and timeline first, then the control-plane skeleton, then read-only observability, each step producing something runnable before the next step starts. BUILD-PLAN owns roadmap sequencing and intended order; Linear owns current execution state, ownership, blockers, and actionable issue tracking. The [build log](BUILD-LOG.md) remains the append-only chronological record through the first pre-release.
...
Nine tracks run in parallel. They are independent enough to build simultaneously, and the dependency that is *not* obvious is called out below.

- **Track A, FPP control.** Step 8 then Step 9. Sequential within itself. Specified.
- **[Track B, nodes and projection](TRACK-B-nodes-and-projection.md).** Governed by [ADR-026](../decisions/ADR-026-renderer-surface-model-and-reference-transport.md) and [ADR-040](../decisions/ADR-040-renderer-extracts-channels-gstreamer-owns-frames.md). **The largest and riskiest track**, and the one carrying reason two for the project. Opened with the transport spike in [`docs/bench/TRACK-B-NDI-SPIKE.md`](../bench/TRACK-B-NDI-SPIKE.md), **which ran 2026-08-16 and passed**: 982,100 frames at 1920x1080, zero dropped, 40.00 fps for 6 h 49 min into Arena over wired NDI. **B2 through B5 are built and merged to `main` (PR #14, 2026-08-17).** `go build ./...`, `go test -race ./...`, `make check`, and `make test-integration` are green on the merge result. A full-track review found 23 findings after every seam's own gates were already green, all fixed before the merge; the pre-merge review found two further cross-track collisions that git had auto-merged cleanly, both fixed. **Four of six acceptance criteria are closed; the two needing real FPP/Resolume/wall hardware are not, and cannot be closed on the development laptop.** The track's one ADR-039 parity gap is now closed: `render.transport.probe` gained its Operator UI control in PR #94 (`00f278e`), a `Probe transport` button on the render surface panel beside apply, clear and restart. It has not been exercised in a browser or against a real node; SM-253 tracks that.
- **[Track C, audio node](TRACK-C-audio-node.md).** **No specific interface gates this track, decided 2026-08-14.** The unit ordered on 2026-08-13 was backordered and interface selection is reopened, but the change is structural rather than circumstantial: [ADR-018](../decisions/ADR-018-program-and-ltc-share-a-clock-domain.md) names a property, not a product, so the engine is built against whatever channel count and output addressing the audio stack reports, and the placement constraint is enforced from advertised capability attributes instead of from a chosen model. Device-independent work proceeds with virtual/null/loopback devices and any available Linux-supported interface. The twenty-minute tone test in [RES-007](../research/RES-007-audio-node-architecture.md) survives as a per-device commissioning check that blocks that interface rather than this track. **Seven audio defect fixes and the positioned-channel-layout fix merged to `main` on 2026-08-24/25**, each gated by `make check` on the merged tree: `f813a8a` (PR #58), `93af06d` (PR #61), `e64a958` (PR #69), `d0ca01e` (PR #72), `29bf1b8` (PR #74), `f0101dc` (PR #75), `3d2883b` (PR #77), and `c75d085` (PR #78, channel positions so the engine can open a positioned sink). **None of it ran on real audio hardware; real-device acceptance stays open**, and PR #61 records that a real-ALSA check was being arranged separately. See the 2026-08-25 entry in [BUILD-LOG.md](BUILD-LOG.md).
- **[Track D, Resolume control and timecode](TRACK-D-resolume.md).** ~~The Resolume adapter over REST, WebSocket and OSC~~ **the Resolume adapter over REST, with one WebSocket as a change signal and no OSC**, plus the LTC timecode path. **Day-0**, promoted 2026-08-13. Its timecode arrives as audio LTC over a physical cable from the audio node. **Track C is no longer on this track's critical path, corrected 2026-08-14:** D0 needs LTC, not ShowMesh's LTC, so any working generator answers RES-001's question about timecode loss. The show's eventual LTC source is Track C's node; benching Resolume's behaviour is not. **The control surface was captured on 2026-08-14** ([docs/bench/resolume-control-surface.md](../bench/resolume-control-surface.md)) and overturned four of this track's own assumptions, including the transport split; the adapter is specified in [TRACK-D-ADAPTER-SPEC.md](TRACK-D-ADAPTER-SPEC.md). **Seam D-1 is built and verified against the running Arena, 2026-08-14** — REST client, WebSocket as a change signal, object-id resolver and the reachability observation, in `internal/coordinator/collector/resolume`. **That build found a defect in Resolume itself: `GET /composition` crashes Arena 7.23.2**, reproduced from `curl` with no ShowMesh running, and the capture's §2.3 leaves it as the only way to enumerate the composition. **Targeted `by-id` reads are safe** (209,916 reads, 6.5 GB, no crash), so the hazard is one endpoint rather than the API. **Every measured crash occurred on the development laptop, which is not the playout machine**: the playout host ran the same composition for a month untouched (see ADR-032's 2026-08-14 revision and BUILD-LOG). **Resolved by [ADR-032](../decisions/ADR-032-resolume-composition-configuration-from-file.md), an owner decision on 2026-08-14: the composition id map is configuration parsed from an uploaded `.avc` file, no runtime path calls `GET /composition` at all, and live state is read by id.** The API enumeration path returns only if a future upgrade proves it safe: the 7.26 upgrade is not scheduled before late November, and the owner struck the vendor report and the soak. **Seam D-2a is that work**: the composition-file parser, the stored id map, the upload API, `showmeshctl` coverage, and the Operator UI upload control, which the owner required shipped rather than noted. **Seam D-2 (observations) closed 2026-08-15 and seam D-3 (the seven-action vocabulary, dispatch, confirmation, the `resolume:action` scope and `showmeshctl` coverage) was built, reviewed and closed the same day**, which is the first thing ShowMesh can do to the wall. **D0, the timecode bench, is untouched and does not gate the adapter.**

**Four more seams built and merged to `track-d/integration` on 2026-08-16: seam B (ADR-037 decisions 1 through 6), seam E, seam C, and D-3a.** All four are on the integration branch with `make check`, `go test -race` and `make test-integration` green on the combined tree, and **merged to `main` on 2026-08-16 after a pre-merge review; none has run against a real Arena since the merge.** **Seam B** makes every operator-facing Resolume reference a name rather than an object id; object ids stay the only addressing on the wire to Arena. Parsing the operator's own `Christmas 25.avc` rather than assuming from the ADR found the vocabulary's own scoping short: deck plus layer plus name still leaves **seven groups covering 16 of 36 clips ambiguous, all on `Main`**, the deck the show runs from, and those refuse by name until renamed. **Seam E** adds `GET /api/v1/resolume/instances` and `/instances/{id}`, the `resolume` snapshot field, the `resolume:<id>` change-stream resource, health through the existing aggregator, and `showmeshctl resolume status` — Resolume observations existed in the store and reached no client before this. **Seam C** adds `resolume` as a third `show.action.target.integration`, so a macro step can drive Resolume by seam B's named-reference vocabulary; the seven D-3 actions were reachable from the API and the CLI and from nothing else before this, which closes an acceptance criterion nobody had noticed was open. **D-3a** ships the crash-recovery seam specified in [TRACK-D-D3A-CRASH-RECOVERY-SPEC.md](TRACK-D-D3A-CRASH-RECOVERY-SPEC.md): the in-memory recovery record, the four-term return gate, the bounded restore with its five skip rules, the `resolume.recovery` toggle and its own dashboard switch, the built-in non-deletable `system-resolume-recovery` principal, and the AST guard keeping process supervision out of core. **D-3a's settle delay (default 8 s) is a stated guess, not a measurement**, and stays that way until it is timed against a real Arena restart. **D-4, the Operator UI, followed the same day and is also built, reviewed and merged to `track-d/integration`** — a Resolume dashboard panel, a `/resolume` view (observations, composition inventory, ambiguous-clip list, crash-recovery record), a name-based action controller, and Resolume authoring in the macro and show-action surfaces. Its diff over `internal/`, `cmd/`, `api/openapi.yaml` and the generated UI schema is empty. Review closed three blocking findings before merge: an unbounded retry loop in `MacroDetail.tsx` measured at roughly 1,200 requests per second from one browser tab; two exported Resolume client-timeout floors asserted against by nothing (the same equal-bound shape D-3a's own review caught hours earlier); and four raw-Arena-object-id leaks on the deck-not-selected refusal path. D-4's real-browser requirement also found a bootstrap regression in D-3a's own code — the reserved recovery principal made `HasAnyPrincipal` permanently true, so no fresh deployment could ever be claimed — fixed separately in `aa56192`. **Track D is now all five seams built, reviewed and merged to `main` (2026-08-16); none has been driven end to end in a browser against a real Arena**, though isolated API-level checks against the real Arena were captured during the D-4 build (see [TRACK-D-resolume.md](TRACK-D-resolume.md)). A local toolchain shadowing (a Node 26.7.0 install ahead of nvm's own pinned Node 22 on `PATH`), unrelated to any of this work and reproducing identically on `main`, was found and resolved the same day; the complete tree now passes `make check`, `go build`, `go vet`, `go test -race` across 27 packages, and `make test-integration` at 236s, detailed in the dated build-log entry. **The pre-merge review found four blocking defects, all fixed on the branch before the fold (`aab7c8e`, `cd6a1ab`); the dated build-log addendum has the record.**
- **[Track E, show authoring and assets](TRACK-E-show-authoring-and-assets.md).** The Show and Surface configuration objects, logical actions, the asset store, and the sync service that gets FSEQ and audio onto nodes. Added 2026-08-13 from the owner's authoring specification, and governed by [ADR-027](../decisions/ADR-027-show-and-surface-model.md) through [ADR-030](../decisions/ADR-030-operator-ui-is-the-authoring-surface.md). **Less new work than it looks:** surfaces were Track B's B3, asset delivery was an open question in both B and C, and logical actions were Track D's D2. This track owns what was floating between them, plus the Show object, the store, and sync. **E1 through E6 are built, reviewed, fixed and acceptance-run, 2026-08-16**, specified in [TRACK-E-SESSION-SPEC.md](TRACK-E-SESSION-SPEC.md): the Show/Surface/active-show configuration kinds, the content-addressed asset store, manual upload, the per-node readiness manifest, and the sync service, all driven end to end through `showmeshctl` with the UI container stopped. Full record in the BUILD-LOG entry below. **E7 stays out until Track D's ADR-037 seam B lands**, because E7's action and binding configuration is exactly what that seam is rewriting. **E8 stays out until D-4 lands**, because E8's screens are D-4's active surface. **FPP Connect compatibility is no longer an October item**: its open decisions were settled on 2026-08-25 in [ADR-044](../decisions/ADR-044-agent-inbound-http-listener.md) and the RES-003 §10 amendment, and the build started 2026-08-26 on the integration branch `dev/fpp-connect`, landing on `main` after the 28 August hardware landing. **Seams FC0, FC1a, FC1b, FC2 and FC3 are built, reviewed and merged to that branch on 2026-08-26 and none of it is on `main`**; FC4, the bench against the owner's real xLights, is the remaining seam and the acceptance gate. See [TRACK-E-FPP-CONNECT.md](TRACK-E-FPP-CONNECT.md) for the current status and seams.
- **[Track F, resting mode and night-session control](TRACK-F-resting-mode.md).** The FPP-authorized rest/show loop, exact-FSEQ-derived timing, independently offset transition cues, final-show handling, and optional configurable interlock/presentation-power sequencing. Added 2026-08-16 from the owner's operating-day workflow and governed by [ADR-038](../decisions/ADR-038-fpp-authorizes-night-sessions.md) and [RESTING-MODE](../architecture/RESTING-MODE.md). It integrates Tracks C, D, and E after their underlying audio, action, and asset seams exist; it does not add calendar scheduling to ShowMesh or require site-control hardware. **F0 through F4 are built, reviewed, gated and committed on `track-f/night-session` (2026-08-18)**: F0's evidence capture found stock FPP has no brightness command at all, which the owner resolved (RES-018, Linear SM-49) as a ShowMesh-owned FPP component not yet built, so F4 implements no brightness fade and readiness rejects a configuration needing one. `make check` and `make test-integration` are green per seam, run by the orchestrating session rather than the builders, plus `make test-integration-fpp` on F3. Review found nine defects on F1, thirteen on F2, ten across two rounds on F3, and sixteen across two rounds on F4, every one of them after that seam's own gates were already green. None of this has run against a real FPP or the deployed fleet. **F5 (audio integration), F6 (optional site control), and F7's UI half are now also built, reviewed, gated and merged to `main`** as PR #51 (`2f35412`), PR #53 (`bd40073`) and PR #45 (`1e839f3`). `pkg/audio`, which F5 depends on, has been present on `main` since `6b0eb38` (2026-08-19). Merging the first two broke `main`'s UI typecheck, because each was green against a base that did not contain the other; repaired by PR #52 (`ea8ea2f`) and the whole tree then verified at `bd40073` with `make check` exit 0 and `make test-integration` exit 0 in 141.851s. F8 remains outstanding and not startable on this machine. See the 2026-08-18 and 2026-08-22 dated entries in [BUILD-LOG.md](BUILD-LOG.md).
- **[Track G, operator surface parity](TRACK-G-surface-parity.md).** Added 2026-08-17 and **scheduled by the owner ahead of Track B**, because Resolume Arena could not be connected to a running coordinator from the UI, the CLI, or the API: the host was an environment variable read once at startup, so connecting one of the three founding problems meant editing a file in the deployment bundle and restarting the container. Governed by [ADR-039](../decisions/ADR-039-operator-configuration-is-store-backed.md). **Every existing rule passed**, because CLAUDE.md's parity constraint and ADR-030 are both conditioned on an endpoint existing, so a capability with zero surfaces owes nothing. The audit behind the track found the same shape in the FPP MQTT collector, the asset store settings, and identity administration, plus `principal:write` declared since Step 6 and checked by no handler. **Seams G-2 (`resolume.instances`), G-5 (identity administration), G-6 (`showmeshctl macro put`) and G-8 (the Operator UI for Track E) are built, gated and open as pull requests #7, #9, #6 and #8**; G-3 (`fpp.mqtt`) and G-4 (`assets.settings`) follow G-2's shape and depend on it merging; **G-7 lands last** and is the point of the track, two tests that fail the build when an operator-facing environment variable is added or a write endpoint ships with no CLI verb. **G-8 was scoped out and back in on the same day** at the owner's direction, and ships as its own pull request so it can be declined independently.
- **[Track H, Cue and Playlist runtime](TRACK-H-cues-and-playlists.md).** Added 2026-08-20 and governed by [ADR-043](../decisions/ADR-043-show-scoped-cues-and-playlist-authority.md). It adds first-class show-owned Cues and Playlists, consumes SM-63's atomic FPP entry identity, deploys active-show-authorized Cue catalogs, and makes rendering, audio, LTC, and announcements consume one activation contract. FPP remains the day-0 scheduler and owns `fpp` Playlist progression; ShowMesh owns only `showmesh-audio` auxiliary Playlist progression. The future general ShowMesh primary runner and calendar scheduler remain out of scope. **H1 through H6 are merged to `main`; H7 is under way and some of its fixes are merged.** H1 (`show.cue` and `show.playlist` configuration kinds) at `69accc0` and H2 (FPP playlist import and reconciliation) at `b5b7f59`; **H3** (a resolved Cue catalog deployed to each node, with anything outside it refused) at `4c107de` (PR #71); **H4** (one Cue activation contract for render, audio and LTC) at `ed9f418` (PR #73); **H5** (a show Playlist as background audio with announcements over it) at `3d7d822` (PR #76). Each was gated by `make check` on the merged tree, and H3, H4 and H5 also by `make test-integration` there. **H6 (Operator UI and readiness) is merged to `main`** as the twenty-two-pull-request Operator UI fold, #91 through #112 excluding #109, squash-merged between 2026-08-25 18:40Z and 2026-08-26 01:30Z (`main` at `53d3db0`): cue authoring and a status/authoring split, playlist authoring and a stored-FPP-playlist-definitions screen, a playlist readiness screen, audio session controls with an Apply control, per-node audio routing, FPP event and playlist detail tables, node signal tables, a render transport probe control, and a readability and navigation pass. `make check` is green on `53d3db0`; browser click-through was obtained for one of the twenty-two pull requests only, and no hardware, deployed-fleet or live-Arena evidence exists for any of them. Six defects found by the verification pass over this fold, including an ADR-011 readability regression and two screens that never render a required response field, are recorded in the 2026-08-25/2026-08-26 [BUILD-LOG.md](BUILD-LOG.md) entry and were fixed by pull requests #119 through #124, taking `main` to `cf0c349`. **H7 (integrated and failure verification) is under way, and every defect its first run filed is now fixed on `main`.** The chain was assembled and run end to end for the first time on 2026-08-26, against running binaries on a containerized FPP 10.0, with the multi-sequence swap proven and two blocking plugin defects fixed and merged upstream (plugin pull request #16, plugin repository `main` `e9b511a`). The defects filed against this repository are all fixed: the coordinator shutdown panic at `4d4fb2d` (PR #138), render content identity at `e2d6274` (PR #137), the later defect against that fix at `cc2f9f8` (PR #149), the per-cue asset gate at `f6923ed` (PR #158), render assignment readiness at `533bbf2` (PR #156), catalog-deploy establishment at `fa6cec9` (PR #168), the superseded render verdict at `5e178a9` (PR #167), and four readiness conditions at `795c48e` (PR #163) and `629754e` (PR #164). Three follow-ups the lane filed rather than fixed remain open: the `assets-missing` readiness condition (unblocked once PR #158 landed, not built), the coordinator/node disagreement about a never-uploaded sequence, and an intermittent supervisor-test race that reddens the Go 1.25 leg on unrelated branches. The bench and its record are merged at `f404ebd` (PR #140). No hardware or real-output evidence exists. See the dated [BUILD-LOG.md](BUILD-LOG.md) entry. Schema v16 was minted by two branches at once during that fold, so H3's acknowledgement table is v17 and H4's `entry_occurrence_sequence` column is v18; see [IDENTIFIER-REGISTER.md](IDENTIFIER-REGISTER.md).
- **[Track I, clock, scheduled start, and the FPP program source](TRACK-I-clock-and-sync.md).** Opened 2026-08-28 and governed by [ADR-046](../decisions/ADR-046-rate-lock-to-a-shared-clock-is-not-chasing.md) and [ADR-047](../decisions/ADR-047-fpp-aes67-is-the-primary-program-source.md), from [RES-019](../research/RES-019-ptp-synchronized-multi-node-audio.md). Audio nodes share one PTP-domain media clock, start on a scheduled instant, take FPP 10's AES67 stream as the primary program source with the local asset as standby, rate-lock each interface at ppm scale, and carry a calibrated per-output offset. Builds on `dev/clock-sync`. **Nothing is built.** The ordering rule is the one to remember: clock provider and scheduled start land before AES67 receive, which lands before the rate lock is required, because the standby is only synchronized if the clock and schedule already exist. Not required for the show to run.
- **Projector power is out.** It stays on Home Assistant and Node-RED, driven by MQTT the way it was from FPP. ShowMesh supplies an external MQTT command step in Step 9 instead, carrying a declared response contract so Node-RED can report the projector actually came on and the step confirms on that evidence. `pkg/pjlink` and ADR-016's provider model are deferred, not cancelled.

### The dependency that will bite, and how it is handled

**Step 9's macros provide finite actions used across the delivery tracks.** Left alone that makes Step 9 block on everything and land in late October, after the show opens. Track F's durable night controller composes those finite actions but is deliberately not one hours-long macro, and Track H does not turn a Cue into another macro language.
...
- **FPP Connect compatibility**, meaning ShowMesh render nodes appearing as their own xLights upload targets so per-node FSEQ files arrive automatically. The owner wants it and scheduled it for **early October** rather than day-0, because the required API surface is unknown and is being researched into RES-003. Day-0 uses manual asset upload, which [ADR-027](../decisions/ADR-027-show-and-surface-model.md) decision 4 and [ADR-028](../decisions/ADR-028-show-asset-store-and-identity.md) decision 8 both make a permanent first-class path rather than a stopgap. Track E is built so this lands as one more ingestion path rather than a redesign. **Superseded 2026-08-25**: the API surface is no longer unknown, RES-003 §10 records it at L1 and [ADR-044](../decisions/ADR-044-agent-inbound-http-listener.md) settles the decisions, so the build was pulled forward. [TRACK-E-FPP-CONNECT.md](TRACK-E-FPP-CONNECT.md) carries the schedule.
- ~~**The Resolume page race.** Layer commands land against whatever page is current … the owner builds the Halloween show on **one page**, which makes the race unreachable … the adapter carries a tripwire.~~ **Retired 2026-08-14 by measurement, and this is not a cut.** "Page" and "deck" are the same thing, confirmed by the owner. The race is real but it is a **clip** race, not a layer race: layer identity is deck-independent, while a positional clip path resolves to a different object on every deck. Since [Track D](TRACK-D-resolume.md) now uses REST `by-id` for everything and offers no positional addressing, **the race has nothing to act on and is removed rather than deferred.** The single-page assumption was also never true: `Christmas 25` already has three decks, so the planned tripwire would have fired on day one. What replaces it is narrower and still fires: an action records whether the selected deck changed between its decision and its confirmation.
...
Status: **complete. Built and acceptance-run 2026-08-14 on branch `step-9-wave-3`, merged to `main` on 2026-08-15.** All 22 acceptance criteria were evidenced against a running coordinator and the bench `fppd`. Three criteria produced owner decisions recorded as ADRs: a run always runs every step ([ADR-035](../decisions/ADR-035-a-run-always-runs-every-step.md)), superseding ADR-031 decision 2's abort default and narrowing ADR-024 decision 11; dispatch configuration applies without a restart ([ADR-036](../decisions/ADR-036-dispatch-configuration-applies-without-a-restart.md)); and the CSRF Origin fallback amendment to ADR-024 decision 6. Those two ADRs were issued as 033 and 034 on the branch and renumbered on merge, because Track D issued ADR-033 (show mode) on `main` the same day. The implementation specification is [STEP-9-SPEC.md](STEP-9-SPEC.md) and its durable decisions are [ADR-031](../decisions/ADR-031-macro-execution-model.md). Its RES-008 prerequisite was discharged on 2026-08-13, and the open question below was answered the same day by the owner: **the first macros ship with no critical local fallback**, stated in the macro definition rather than left implicit, because the delivery path does not exist and building it is cut from day-0. Revisit after Halloween. **This step is day-0 scope**, not optional: show macros are one of the reasons the project was started.

**Superseded 2026-08-18 by the approved three-repository target, extended 2026-08-20 by ADR-043.** `showmesh` retains coordinator/API/UI/CLI/collector work; `fpp-showmesh` is the thin Plugin Manager package; `showmesh-fpp-plugin` owns the Go macro helper, resident C++ brightness and playlist-observation runtime, tests, and release artifacts. The current checkout has not been extracted yet: the Go source remains at `cmd/showmesh-fpp-plugin/`, the packaging repository is a local sibling, and the runtime repository does not exist. [RES-018](../research/RES-018-fpp-brightness-control.md) is the build record.

**Owner decision, 2026-08-15, retained under the new split: release artifacts stay unpublished until the first real-host install.** Today this repository builds and self-verifies the Go helper for all three architectures and publishes nothing. After extraction, `showmesh-fpp-plugin` owns those binaries plus the C++ source bundle; `fpp-showmesh` pins their exact filenames and SHA-256 values in a committed lock file. The real-host install and publication gates remain open.

**One Step 9 follow-up remains open and does not block this step.** The `Sec-Fetch-Site`/Origin fallback was verified in Chrome on macOS only, and the phone is the device that fix exists for. The former artifact-URL ambiguity is answered by RES-018: `fpp-showmesh` will download locked assets from `showmesh-fpp-plugin`, not this monorepo. That change belongs to SM-63 because the target release does not exist yet.
...
- **An external MQTT command step type with a declared response contract**, added 2026-08-13 when the owner cut projector control from day-0. A macro step publishes an operator-authored payload to an operator-authored topic, so Home Assistant and Node-RED keep driving the projectors exactly as they did from FPP. This replaces `pkg/pjlink` and the ADR-016 provider model for day-0 and buys back everything both would have cost.

  **It is a confirmable command, not an escape hatch**, which is the owner's refinement on 2026-08-13 and a better design than the first draft of this bullet. Node-RED can publish a status back when it sees the projector actually come on, so the step declares **what response it expects**: nothing, a boolean, a number, text, or a matched value, on a named topic, within a deadline. Confirmation is then a real evidence check against a contract the operator wrote, which is precisely what [ADR-003](../decisions/ADR-003-desired-and-observed-state.md) asks for when it says a command declares how success will be confirmed.

  **Expecting nothing stays legal and stays honest.** A step configured with no expected response is genuinely unconfirmable, and reports as unconfirmable with a reason from [ADR-020](../decisions/ADR-020-control-api-shape-and-change-stream.md)'s vocabulary rather than as success, because a macro step that always reports success is worse than no step: the operator stops reading it, and then it is silently useless on the night it matters.

  **This is the general integration point, and it should be built as one.** It is not a projector feature. It is how ShowMesh calls anything reachable over MQTT and confirms by a declared contract, which is why it earns a real design rather than a hardcoded publish. It is also the one step type deliberately exempt from Step 8's rule about only shipping commands whose effect ShowMesh itself observes: here the operator supplies the observation, and the exemption is recorded rather than assumed.
...
These deliberately come later, and why. **Entries sequenced into a track on 2026-08-13 are marked as such rather than deleted**, because each one's reasoning about what gates it is still the reason its track is shaped the way it is.
...
- **Controlled devices and control providers stay unsequenced, and projector power was cut from day-0 entirely** on 2026-08-13: Home Assistant and Node-RED keep driving the projectors over MQTT. [ADR-016](../decisions/ADR-016-controlled-devices-and-control-providers.md) settles the model; the metadata contract and the metadata-generated-surface hypothesis are unresearched in [RES-014](../research/RES-014-control-provider-model.md), and the first provider (projectors, `pkg/pjlink`) also depends on RES-012 bench work.
- **Audio engine and audio node are now [Track C](TRACK-C-audio-node.md)**, sequenced 2026-08-13 and no longer in this list, because day-0 scope includes audio and because the audio node is also Resolume's LTC source. The entry below is kept for its reasoning, which still holds: the architecture is decided ([ADR-017](../decisions/ADR-017-showmesh-owns-audience-audio.md), [ADR-018](../decisions/ADR-018-program-and-ltc-share-a-clock-domain.md), [ADR-019](../decisions/ADR-019-audio-device-loss-fails-silent.md), [AUDIO-ENGINE.md](../architecture/AUDIO-ENGINE.md)) and entirely unverified. It was previously unsequenced because [RES-007](../research/RES-007-audio-node-architecture.md) is critical-risk at L0 and because the multichannel interface had not been purchased. **The second reason was removed twice, and the second removal is the durable one.** A unit was ordered on 2026-08-13; it was backordered, and on 2026-08-14 the track was rebuilt so that **no specific interface gates it at all**, against whatever channel count and output addressing the audio stack reports. The first reason stands, and nothing here can be raised above L0 by unit tests, since whether GStreamer holds LTC sample-aligned to program, and what drift a free-running node accumulates over a show, are bench facts. The RES-007 prototype starts with its device-independent software stage on a representative Linux host; per-device commissioning follows when a candidate show interface is available. ADR-018 remains a purchasing constraint of at least three output channels from one clock domain, now enforced from advertised capability attributes rather than from a chosen model, with the per-device tone test as commissioning.
- **The Resolume adapter is now [Track D](TRACK-D-resolume.md)**, sequenced 2026-08-13 and no longer in this list, because controlling Resolume is one of the three founding problems. Track D is building: **D-1, D-2 and D-3 are built, and D-3 was reviewed and closed 2026-08-15**, so ShowMesh can now act on Resolume. **The build order is settled (owner, 2026-08-16): [ADR-037](../decisions/ADR-037-resolume-references-are-names-not-ids.md) seam B, then [D-3a](TRACK-D-D3A-CRASH-RECOVERY-SPEC.md) crash recovery, then D-4 (the Operator UI).** ADR-037 comes first because references become names rather than object ids, and both later seams put operator-facing text on a screen. **D-2 and D-3 were driven against a real Arena 2026-08-15**, thirteen checks passed and none failed ([results](../bench/TRACK-D-LIVE-VERIFICATION.md) §6), on the development laptop rather than the playout machine. **D-3a has no owner decisions outstanding**: a pre-build review on 2026-08-16 raised six and all six were answered the same day, recorded as that specification's §10. Two of them changed its scope: the restore covers the layers ShowMesh drove explicitly rather than the timeline, and the recovery toggle's UI switch ships in D-3a rather than D-4. D0 (the RES-001 timecode bench) does not gate the adapter; it answers timecode-fault behaviour, which is a separate half of the track. RES-001's control/observability half reached L2 on 2026-08-14 from the capture; its fault behaviour stays L0 until D0 runs. **All five seams — B, E, C, D-3a and D-4 — were built, reviewed and merged to `track-d/integration` on 2026-08-16**, gated by the combined tree's `make check`, `go test -race` (27 packages, 0 failures) and `make test-integration` (236s); see the dated build-log entries and [TRACK-D-resolume.md](TRACK-D-resolume.md) for what each closed. **D-4 is the last seam Track D owed**: a Resolume dashboard panel, a `/resolume` view, a name-based action controller, and Resolume authoring in the macro and show-action surfaces, with an empty diff over `internal/`, `cmd/` and `api/openapi.yaml`. Its own real-browser requirement found a bootstrap regression in D-3a's built-in recovery principal — it made `Store.HasAnyPrincipal` permanently true, so no fresh deployment could ever be claimed — fixed separately in `aa56192`. **None of the five has merged to `main`, and none has been driven end to end in a browser against a real Arena**, though isolated API-level evidence against the real Arena was captured during the D-4 build. The next action is the owner sequencing this branch against Track E's PR #3 and the uncommitted Track F work in the main checkout.
- **GStreamer pipeline supervision and NDI transport are now [Track B](TRACK-B-nodes-and-projection.md)**, sequenced 2026-08-13 and no longer in this list. B1 shipped 2026-08-14. The frame-rate, pacing, and alignment numbers are produced by assembling this path, not prerequisites for starting it; RES-004 and RES-005 are filled from the integrated run. The one genuinely gating question was the NDI runtime and distribution answer in RES-006, and **B0's spike settled it on 2026-08-16**: NDI holds on the wired path at the reference dimensions, and the runtime question resolved to a source-built `gst-plugins-rs` plugin rather than a packaged one. Nothing in Track B is gated on research now.
- **Preview delivery.** Cut from day-0; this file's day-0 schedule already says so. It blocks no other work.
- **Pixel-current diagnostics.** Collection shipped in Step 5. RES-011 cannot rise until the display is energized, which is seasonal, and this blocks no other work.

## Standing constraints for every step

- FPP stays authoritative for calendar time and the operating window. Per ADR-038, ShowMesh may advance individual rest/show cycles inside an FPP-authorized night session using authored-media duration and relative cues; it never evaluates a calendar schedule.

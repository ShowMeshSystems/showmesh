...
- **[Track B, nodes and projection](TRACK-B-nodes-and-projection.md).** Governed by [ADR-026](../decisions/ADR-026-renderer-surface-model-and-reference-transport.md) and [ADR-040](../decisions/ADR-040-renderer-extracts-channels-gstreamer-owns-frames.md). **The largest and riskiest track**, and the one carrying reason two for the project. Opened with the transport spike in [`docs/bench/TRACK-B-NDI-SPIKE.md`](../bench/TRACK-B-NDI-SPIKE.md), **which ran 2026-08-16 and passed**: 982,100 frames at 1920x1080, zero dropped, 40.00 fps for 6 h 49 min into Arena over wired NDI. **B2 through B5 are built and gated on `track-b/integration` at `b1789ac`, 2026-08-17, and not yet merged to `main`.** `go build ./...`, `go test -race ./...` (33 packages), `make check`, and `make test-integration` all green on that commit. A full-track review found 23 findings after every seam's own gates were already green — see the 2026-08-17 BUILD-LOG entry — all fixed at `b1789ac`. **Four of six acceptance criteria are closed; the two needing real FPP/Resolume/wall hardware are not, and cannot be closed on the development laptop.** Next action is the orchestrator's pre-merge review and fold, the same discipline Tracks D and E went through on 2026-08-16.

...
- **The Resolume adapter is now [Track D](TRACK-D-resolume.md)**, sequenced 2026-08-13 and no longer in this list, because controlling Resolume is one of the three founding problems. Track D is building: **D-1, D-2 and D-3 are built, and D-3 was reviewed and closed 2026-08-15**, so ShowMesh can now act on Resolume. Crash recovery and D-4 (the Operator UI) are next. **D-2 and D-3 were driven against a real Arena 2026-08-15**, thirteen checks passed and none failed ([results](../bench/TRACK-D-LIVE-VERIFICATION.md) §6), on the development laptop rather than the playout machine. **[ADR-037](../decisions/ADR-037-resolume-references-are-names-not-ids.md) is the next build item**: references become names rather than object ids. D0 (the RES-001 timecode bench) does not gate the adapter; it answers timecode-fault behaviour, which is a separate half of the track. RES-001's control/observability half reached L2 on 2026-08-14 from the capture; its fault behaviour stays L0 until D0 runs.

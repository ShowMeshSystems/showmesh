...
- **[Track D, Resolume control and timecode](TRACK-D-resolume.md).** The Resolume adapter over REST, WebSocket and OSC, plus the LTC timecode path. **Day-0**, promoted 2026-08-13. Its timecode arrives as audio LTC over a physical cable from the audio node. **Track C is no longer on this track's critical path, corrected 2026-08-14:** D0 needs LTC, not ShowMesh's LTC, so any working generator answers RES-001's question about timecode loss. The show's eventual LTC source is Track C's node; benching Resolume's behaviour is not.
...
- **The Resolume page race.** Layer commands land against whatever page is current, and Resolume cannot pin a layer to a page. The owner builds the Halloween show on **one page**, which makes the race unreachable, and the fix waits until the project is out of hard build mode. **The [Track D](TRACK-D-resolume.md) adapter carries a tripwire rather than a reminder:** a composition with more than one page surfaces as visible evidence, because that is the exact moment the assumption stops holding and nobody adding a second page in December will be reading a planning document.

...
- **[Track H, Cue and Playlist runtime](TRACK-H-cues-and-playlists.md).** Added 2026-08-20 and governed by [ADR-043](../decisions/ADR-043-show-scoped-cues-and-playlist-authority.md). It adds first-class show-owned Cues and Playlists, consumes SM-63's atomic FPP entry identity, deploys active-show-authorized Cue catalogs, and makes rendering, audio, LTC, and announcements consume one activation contract. FPP remains the day-0 scheduler and owns `fpp` Playlist progression; ShowMesh owns only `showmesh-audio` auxiliary Playlist progression. The future general ShowMesh primary runner and calendar scheduler remain out of scope. **H1 through H5 are merged to `main` as of 2026-08-25.** H1 (`show.cue` and `show.playlist` configuration kinds) at `69accc0` and H2 (FPP playlist import and reconciliation) at `b5b7f59`; **H3** (a resolved Cue catalog deployed to each node, with anything outside it refused) at `4c107de` (PR #71); **H4** (one Cue activation contract for render, audio and LTC) at `ed9f418` (PR #73); **H5** (a show Playlist as background audio with announcements over it) at `3d7d822` (PR #76). Each was gated by `make check` on the merged tree, and H3, H4 and H5 also by `make test-integration` there. **H6 (Operator UI and readiness) and H7 (integrated and failure verification) are not started.** Schema v16 was minted by two branches at once during that fold, so H3's acknowledgement table is v17 and H4's `entry_occurrence_sequence` column is v18; see [IDENTIFIER-REGISTER.md](IDENTIFIER-REGISTER.md).

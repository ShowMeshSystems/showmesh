...
Seven tracks run in parallel. They are independent enough to build simultaneously, and the dependency that is *not* obvious is called out below.
...
**Step 9's macros provide finite actions used across all six tracks, and most of those tracks do not exist yet.** Left alone that makes Step 9 block on everything and land in late October, after the show opens. Track F's durable night controller composes those finite actions but is deliberately not one hours-long macro.
...
**The FPP plugin's packaging lives in a separate repository**, currently the untracked local `fpp-showmesh` alongside this one: `pluginInfo.json`, the FPP command definitions, the install/upgrade/uninstall/preStart scripts, and its own bench capture. The plugin's Go source is in this repository, at `cmd/showmesh-fpp-plugin/`. **Owner decision, 2026-08-15: its long-term home is its own public repository under `ShowMeshSystems`**, matching FPP's convention that a plugin is a standalone repository the plugin manager clones by URL. That repository does not exist yet.

**Owner decision, 2026-08-15: release artifacts stay unpublished until the first real-host install.** CI builds and self-verifies all three architectures and deliberately publishes nothing; installs use `SHOWMESH_PLUGIN_ARTIFACT_BASE_URL` pointed at a bench path. The installer's default URL therefore resolves to a release that does not exist. That is a named deferral, not an oversight, and it pairs with the standing decision that real-host install stays deferred and RES-015 stays L1.

**Two follow-ups are open and neither blocks this step.** The `Sec-Fetch-Site`/Origin fallback was verified in Chrome on macOS only, and the phone is the device that fix exists for. And `scripts/lib/fetch.sh`'s default artifact URL points at this monorepo's releases, which is either a deliberate cross-repo split or wrong once the packaging repository exists; it costs nothing while the bench override is what actually runs.

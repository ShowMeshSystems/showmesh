...
**Track F seam F4 and Track C, both 2026-08-18, three instances in one night across two tracks, every one found by review after the seam's own gates were green.**
...
Track C hit the same shape twice in its own seams, where a mechanism reported success in exactly the case it had not worked.
...
Two properties tell you the identity is too coarse, and both are checkable at design time rather than after a defect. First, ask whether two legitimately different pieces of work can produce equal keys, and check it against the validation that is supposed to prevent it, including which *scope* that validation runs in. Track F's uniqueness check was real and enforced per list, which is exactly why it read as sufficient. Second, ask what happens on the collision: if the answer is "returns the existing record", the collision is silent by construction and no test that exercises one list will ever see it.

**Rule:** when persisting an outcome against an identity, write down what the identity is claiming to be unique across, then find the enclosing scope that validation does not cover. Test the collision directly, with two genuinely different pieces of work that should not alias, and assert the second one actually ran rather than asserting that the call returned successfully. A uniqueness constraint enforced in a narrower scope than the identity is used in is not a constraint, and its failure mode is a green report for work that never happened.

...
## A dependency's version is part of the deployment environment, and the dev machine's newer one hides that

**Track B, PR #14 merge, 2026-08-17.** The renderer builds a `gst-launch-1.0` argv. Two of the arguments it passed do not exist on the GStreamer that Ubuntu 24.04 ships. `fdsrc`'s `is-live` property was added upstream in 1.26, and `rawvideoparse`'s `format` property wants the lower-case `GstVideoFormat` nick (`rgb`) rather than the upper-case caps spelling (`RGB`) that the same value correctly carries inside a caps string. Measured: 1.28.6 on the development laptop accepts both; 1.24.2 rejects each at pipeline construction, with `no property "is-live" in element "fdsrc"` and `could not set property "format" in element "rawvideoparse" to "RGB"`.

Neither is a state-change failure, so neither produces a useful error at the point of use: the pipeline is rejected before it starts, the supervisor crash-loops, and the node reports `gst_launch_available: true` the whole time. Every local gate passed on all five checks precisely because this laptop has the newer GStreamer.

The second one is the sharper case. `RGB` and `rgb` are two different vocabularies, a caps-string spelling and an element-property nick, that happen to coincide on newer GStreamer. One mapping function served both call sites, so the distinction was invisible until a version that enforces it. The capability probe made it worse: `detectRenderSurfaceFormats` probed with a caps string, which succeeds on 1.24, so the node advertised `render.surface` supporting `rgb` and then failed every apply. That is this repository's recurring "advertises a capability nothing can reach" shape, arriving through a version difference rather than through missing wiring.

**Rule:** an external binary's version is part of the deployment environment, not a detail of the developer's machine. When code emits arguments, properties, or syntax for another program, the version that matters is the one on the target host, and a newer local version silently accepts things the target rejects. Probe the capability at runtime and build accordingly rather than emitting the newest form and assuming; where two vocabularies coincide on your version, encode both spellings in one place so a later addition cannot pick the wrong one. And a probe that does not exercise what the real path builds is not evidence the real path works.

## Full-stack running on the target hardware found what CI structurally could not, and the same bug twice by two routes

**Track B, 2026-08-17, owner running the renderer on the real render host with FPP in Docker.** The same evening PR #14's CI caught the `rawvideoparse` nick defect on a Linux runner, the owner hit and fixed the identical defect by running the assembled system on the hardware it will run on. Two independent routes, one bug, converging.

What only the live run found: a render node that did not answer MultiSync discovery or announce an address FPP could actually use, and a stride that did not match the packed rows the frame writer emits. Neither is reachable from CI. The first needs a real FPP on a real network deciding whether it believes the node exists. The second needs real frames with real geometry moving through the real pipeline, and is specifically hidden by a test pattern: a uniform synthetic field is exactly the input a stride error does not visibly corrupt. Roughly 45 minutes of live work, against defects no unit test, no bench container, and no CI job was positioned to see.

This is not an argument against the CI coverage added in the same session. CI now catches the version-skew class for free on every push, which is the class that costs a human 45 minutes to rediscover by hand. It is an argument about what each is for: CI cheaply re-catches known shapes, and running the whole thing on the real hardware is the only thing that finds the shapes nobody has named yet.

**Rule:** CLAUDE.md's "when they compete for time, integrate" holds, and this is the measured case for it. Prefer assembling the system on its real hardware over proving each part in isolation first, and treat a synthetic test input as a known blind spot rather than as coverage: a test pattern is chosen for being uniform, which is the same property that makes geometry, stride, and alignment defects invisible in it.

## A mitigation built on an unverified diagnosis can make the failure worse

**Track B, PR #14 CI, 2026-08-17.** Installing GStreamer on the CI runner took four attempts, and three failed on a wrong diagnosis rather than on the actual cause. The install was first assumed to need `gstreamer1.0-plugins-bad` for `rawvideoparse`, which on 1.24 lives in `plugins-base`; the unnecessary package pulled in pipewire, jack, SDL2 and a 5 MB soundfont and took the step from about 3 seconds to over 300. A hang was then diagnosed as dpkg lock contention and mitigated with `DPkg::Lock::Timeout`, when the logs showed it stalling mid index-fetch. Finally, `Acquire::Retries=3` was added to make the fetch "more robust" against a runner mirror that had stopped answering, which lengthened the stall it was meant to shorten: three retries at a 30 second timeout on every index file, when apt's own failover to the working mirror needed only to be reached sooner.
...
## A tri-state built to stop a known defect reproduced it on its own untested side

**Track C seam C0a-2, 2026-08-18.** `pkg/audio`'s `Field[T]` exists specifically to give a write payload three distinguishable states — unset, explicit null, and a real value — because this project has already found the two-state version of this bug twice, both times as an absent JSON key silently wiping data (`SHOWMESH_FPP_ENDPOINTS`'s missing `endpoints` key, and a `declare` with no label). The decode side was built and tested correctly: an absent key leaves `FieldUnset` untouched, since `encoding/json` never calls `UnmarshalJSON` for a key that was never in the source object. Review found that the encode side had not been held to the same standard — `MarshalJSON` emitted JSON `null` for an unmentioned, `FieldUnset` field, indistinguishable on the wire from an explicit clear. An `audio.session.apply` meant only to raise gain, marshalled through that path, would have wiped the pinned playlist, ceiling, mix policy, and output set and reported success. This is the SHOWMESH_FPP_ENDPOINTS shape a third time, now found inside the very type built to prevent it from recurring. Fixed: `MarshalJSON` now returns `ErrFieldUnsetMarshal` for `FieldUnset`, forcing every wire side to build its own optional-shaped payload rather than risk the silent `null`.

**Rule:** a tri-state type is only tri-state on the side that was actually tested. The decode side is the one that gets attention, because it is the side a hostile or careless caller exercises first; the encode side is written once, by the type's own author, on the assumption that "we built it correctly" extends to both directions. It does not, and it has to be checked and tested separately.

## A metric that reads identically for the two cases it exists to tell apart

**Track C seam C0a-1, 2026-08-18.** R4 set out to measure whether a 200 ms gain fade is click-free versus an abrupt gain step, using the most direct metric available: the raw sample-to-sample delta across the transition. It came back useless — both the ramped fade and the abrupt step measured `max_delta: 3421`, exactly identical — because a periodic test tone's own per-sample slope can exceed either transition's own contribution to that same delta, so the metric was dominated by the signal rather than by the thing being measured. Switching to a windowed-envelope discontinuity metric (`envelope_max_step`, computed over roughly one cycle) separated them cleanly: 158 for the ramp, 20978 for the step, about 133x apart.

**Rule:** before trusting a metric, check it against the case it exists to reject, not only the case it exists to confirm. A metric that cannot produce two different numbers for two genuinely different inputs is not measuring the property under test; it is measuring something else that happens to correlate with success in the easy case. This generalizes past audio: any pass/fail check built from a single scalar comparison should be run once against the input it is supposed to fail, before it is trusted to pass the input it is supposed to accept.

## A well-tested function with no caller is a decoration, not a guarantee

**Track C seam C0a-2, 2026-08-18.** `ApplyCeiling` shipped with complete tests covering clamping, non-clamping, and invalid-input cases, and nothing in `pkg/audio` called it: a session's gain ceiling was validated in isolation and never actually enforced against a session's effective gain, because nothing wired `ApplyCeiling` into the merge path that produces a session's desired state. The maximum-gain safety property it exists to guarantee was decorative until review found it and `ApplyRequest.Merge` was given the call, reporting the clamp outcome in `MergeReport` rather than silently storing a clamped or unclamped value.

Same shape as Step 6's three unreachable features (`ClaimBootstrap`, `IssueToken`, `CreatePrincipal`, each fully tested and callable by nothing that existed) and Track D's D-3a crash-restore code, which was implemented, tested, and wired by nothing.

**Rule:** tests prove behaviour, never reachability. A passing test suite says a function does what it claims when called; it says nothing about whether anything in the shipped system ever calls it. When a step adds a capability meant to enforce an invariant, check for a real caller on the path that invariant is supposed to protect before calling the capability done.

## A diagnostic that exercises a real output is itself show-affecting

**Track C seams C1a/C1b, 2026-08-18.** The capability probe built `audiotestsrc` with `wave=sine`, whose `volume` defaults to 0.8, and the periodic report loop re-ran full discovery every fifteen seconds. A commissioned node would have emitted roughly a second of a -2 dBFS 440 Hz tone out of every discovered output, every fifteen seconds, during the show, including into the FM transmitter ADR-019 exists to protect. Measured: peak absolute sample 26,213 (roughly -2 dBFS) from the pre-fix pipeline, against peak absolute sample 0 across 51,200 frames (1.07 s at 48 kHz) from the fixed, silent probe.

The C0a-1 bench could not have caught this, structurally rather than by bad luck: it ran against a null ALSA device, which makes no sound regardless of what the pipeline is asked to emit, and every measurement it took was of a captured file, never of an output. A bench built entirely on virtual devices measures the code's logic — did the pipeline construct, did it reach PLAYING, did the graph route samples where intended — and says nothing about what an audience standing near a real output would hear.

**Rule:** a diagnostic or capability probe that touches a real output is itself show-affecting and needs the same scrutiny as a playback command, not the lighter scrutiny a read-only check usually earns. And a bench run entirely on virtual/null devices is evidence about pipeline construction and graph logic; it is not evidence about what a real output does, no matter how many times it passes.

## A setting that reached the store and stopped

**Track C seams C1a/C1b, 2026-08-18.** The agent stamped a hardcoded `"undeclared"` clock domain into its own capabilities and its own report, while `audio.node` — the configuration kind built specifically to supply that value — had no reader anywhere in the codebase. An operator could `PUT /api/v1/config/audio.node/{id}` with a real clock domain, get a `200`, a revision, and an audit entry, and the evidence surface would report `undeclared` forever: a permanently contradicting pair, one asserting a fact and the other asserting its absence, both live at once.

This is the second time this project has hit this exact shape under ADR-039: Track B's `idleOutput` was the first, a capability the operator could configure with nothing downstream that read it. The fix worth generalizing is the *direction* one, not just the wiring one. A node cannot observe whether two of its own outputs share a hardware clock — that is not evidence a probe can produce. So the node stopped claiming the property, and the coordinator now emits both the clock-domain and clock-domain-provenance signals from its own store, where the operator's declaration actually lives, reporting `not_collected` with a reason where nothing is declared, rather than inventing a distribution mechanism to carry an unverifiable value down to a node that has no way to check it.

**Rule:** before wiring a configuration kind's value into a node's own advertised evidence, ask whether the node can actually observe the property the value describes. If it cannot, the coordinator emitting the value from its own store — with an honest `not_collected` when nothing is declared — is the correct shape, not a distribution path built to hand an unverifiable claim to the one place least able to check it. And ADR-039's store-and-stop failure (a configuration kind with no reader) is now a named recurring shape, not a one-off: check for a real reader before calling a configuration kind done, the same way a real caller is checked for a function.

## Describing a contract accurately is a way of auditing behaviour

**Track C seam C3, 2026-08-18.** `internal/coordinator/api/audiodispatch.go` registered nine `audio.session.*` endpoints, and `api/openapi.yaml` described none of them — the ADR-014 half that only bites once a client generator or a conformance test discovers the routes later. Writing the missing entries was not transcription. Documenting the `409` response forced the question "what actually produces a 409 here?", and the honest answer was "nothing": a reused idempotency key was answered as a replay without ever checking whether the existing command's action and params matched the new request, and replay responses read the outcome from the generic `commands` table rather than the session-specific outcome that had actually been dispatched. Both were real defects, found by neither a test nor a review pass, but by the act of writing an accurate contract entry and being unable to justify it. The same missing action/params check is still open on render dispatch (Linear SM-60), so this is a shape recurring across seams, not a one-off caught once.

The builder also deliberately did **not** document a `404` for these paths, because nothing in the handler produces one — the same discipline in the other direction: a response only belongs in the contract if something real returns it.

**Rule:** write the contract entry at the same time as the endpoint, not afterward as cleanup. When documenting a response, verify something in the code actually produces it before adding it — a plausible-looking entry added because it "completes" the contract is exactly how an uncheckable claim gets into a public interface. Treat the exercise of writing the contract as an audit of the behaviour it describes, not merely a record of it.

## A test must exercise the ordinary case, not only the elaborate one

**Track C seams C6/C7, 2026-08-18.** Two instances in one pass. First: the existing natural-completion test covered only a two-item playlist, so the single-media session — the ordinary shape, arguably more common than a playlist in this system's actual show configurations — was never exercised, and it was the one that shipped broken. `advanceLocked` refused unconditionally when a session had no playlist; the natural-completion watcher calls that same method on completion, so a finished single-asset session reported `state=playing` forever, position pinned at the asset's full duration, measured unchanged for 25+ seconds. Track F anchors night-loop transitions on observed completion, so a background track that never reports completing is a night loop waiting for a cue that cannot arrive.

Second, in `pkg/audio`'s own contract seam: `TestFieldMarshalRoundTrip` never unmarshalled anything and never marshalled an *unset* `Field[T]`, which was precisely the input that exposed the tri-state encode bug (an earlier lesson in this same file covers the decode side of that type; this is the encode side going unexercised in turn).

**Rule:** a test's fixture selection is itself a claim about which case matters. Both of these chose the elaborate case — the two-item playlist, the populated field — because it is more interesting to write, leaving the suite green over the path that actually ships most often. Before trusting a suite's coverage of a state machine or a wire type, ask which fixture is the ordinary one in production and check that it, specifically, has a test.

## A defect that only a real backend can produce, hidden by a fake that is too clean

**Track C seams C6/C7, 2026-08-18.** Fade completion compared the engine's reported gain to its target with exact float equality. A real backend computing a ramp reports `0.19999999999999998` for a target of `0.2`, misses the equality check, and reports `unconfirmable` forever — which would have made Track F's transition barrier permanently unreachable the day a real engine arrived, since the barrier waits on `fade_complete`. `internal/agent/audio/fakeengine.go` stores exact values, so no test written against it, however thorough, could reach the defect; it was found and fixed as part of the same pass that added the fault vocabulary, not by a test discovering it.

This is the same shape as the C0a-1 bench's null audio device, which stays silent no matter what the pipeline is asked to emit and so cannot tell a click-free fade from an abrupt cut: a substrate clean enough to be convenient is often too clean to expose the defect the real thing would produce, and the defect then waits, undetected, for hardware or a real backend to arrive.

**Rule:** when a fake or null substrate stores or reproduces values exactly, ask what class of defect only shows up when the real thing computes those values instead — floating-point drift, timing jitter, quantization, negotiated versus requested parameters — and inject that class synthetically (a tolerance test, a mutation test, a fixture built from measured real-world noise) rather than waiting for the real backend to surface it on its own schedule.
...
## An identity that is not unique reports success for work that never happened

**Track F seam F4 and Track C seam C3, both 2026-08-18, found by review after each seam's own gates were green.**

Track F's night-session cue outbox keyed a cue's durable identity on session, cycle and cue name. `enterShow` and `enterResting` are validated for name uniqueness separately, and both run under the same cycle, so a `blackout` cue in each list shared one identity. The runner found the first list's row already resolved, returned it unchanged, and reported the *second* cue's outcome as the first one's. Measured: the enterResting fade-up never dispatched, and the run recorded it as succeeded. The identity now carries the phase.

Track C's audio session layer hit the same structure from the other direction. `advanceLocked` refused unconditionally when a session had no playlist, and the natural-completion watcher called that same method, so a single-media session reported `state=playing` forever with position pinned at the asset's full duration, measured at 2000 ms on a 2.0 second asset and unchanged for 25 seconds and counting. The guard was correct for the caller it was written for and wrong for the caller it later acquired.

The common structure is not a typo in a key. It is that **a durable identity is a claim about what counts as the same piece of work**, and when that claim is too coarse, the second piece of work is not merely lost, it inherits the first one's outcome. That is worse than a dropped record, because a dropped record shows up as missing while an aliased one shows up as done. Every mechanism in this project that persists an outcome against an identity has this failure available to it: command idempotency keys, the cue outbox, asset identity, session invocation ids.

**Both checks were genuinely correct where they were written, which is why both read as sufficient to their reviewers.** Track F's uniqueness rule was real and enforced, in a narrower scope than the identity was used in. Track C's precondition was right for its original caller, and a second caller arrived later. In neither case did anyone write a wrong check; in both cases the set of paths reaching the check changed and nothing re-asked whether it still covered them.
...
## A flag one watcher writes and nobody reads on the call path lets a dead subsystem keep answering with the state it last held

**Track C, 2026-08-20.** The bus watcher on the audio agent's shared output pipeline correctly detected the pipeline had broken and set a flag recording it. Nothing on the per-handle command path read that flag. So `Observe` on a session whose output had died kept returning `playing`, because the code computing the answer had no idea the flag existed, only that the session's last known state was `playing`. Found by the orchestrator in an adversarial pass, not by a test: Resume, Seek, and Start all returned "playing"; Stop, SetGain, and Observe all returned "stopped"; Pause returned "paused"; every one of them with no error, on a pipeline that was in fact dead.

The second half is the sharper one. The session layer already has an availability gate that rewrites command *outcomes* when the engine is unavailable, and that gate did the right thing everywhere it applied. It does not touch the retained telemetry snapshot, which is a second, separately-read surface carrying its own copy of session state. A guard written for one surface does not cover a second surface just because both surfaces derive from the same underlying session, and nothing said so until this was checked by hand.

**Rule:** a flag set by a watcher is not a guard until something on every read and write path actually checks it. When a subsystem can go stale or die out from under a session, enumerate every surface that reports that session's state, not just the one the existing guard was written against, and confirm each one either reads the liveness flag or is fed from a source that already has. The fix here put the check at `branchFor`, the one choke point every handle-addressed method already passes through, specifically so a future new method inherits the guard rather than needing to remember it.

## A persistence gate loosened from an allowlist to a `default: true` switch let an unrecognised value silently become success

**Track C, 2026-08-20.** Desired state previously persisted only when a dispatch's outcome was `confirmed`, too narrow a gate, since `unconfirmable` is the ordinary result of an audio session dispatch and almost nothing ever persisted. The fix widened the gate to persist on any outcome meaning the command reached the node. Read as a diff, that reads like a straightforward allowlist expansion. The actual shape it landed in matters: the check became "persist unless the outcome is one of the small set that means the command never arrived," with everything else, including any outcome value the coordinator does not itself recognise, falling through to the persisting branch by default.

That is this project's standing rule about unknown never collapsing into success, reappearing on a gate that exists specifically to record whether something happened. An allowlist and a denylist are not interchangeable ways of writing the same rule: an allowlist treats an unrecognised value as unproven and refuses to act on it, while a denylist-shaped `default: true` treats an unrecognised value as proven and acts on it, and the two differ exactly on the case that matters, which is the value nobody anticipated.

**Rule:** when a gate decides whether to persist, dispatch, or otherwise act on an outcome, write it as an allowlist of the values that justify acting, not a denylist of the values that don't. An unrecognised value must fall on the refusing side by construction, not survive by falling through every named exception. This is the `unknown`/`unsupported`/`unconfirmable`/`not_ready`/`failed` rule in a new disguise: check any gate that was recently "loosened" for whether the loosening happened by widening an allowlist or by inverting to a default-permissive switch, because the second shape is where a value nobody named gets treated as a value somebody approved.

## A replaced component keeps whatever it was holding until something closes it, and a test double holds nothing

**Track C, 2026-08-20.** The audio agent rebinds its playback engine whenever its `audio.node` configuration changes, so the engine can follow a route change without restarting the agent. The rebind invalidated every session in flight, swapped the backing engine, and never closed the engine it replaced. A GStreamer engine holds its output pipeline in `PLAYING` and its ALSA device open until something closes it, so every configuration revision would have leaked a pipeline, a bus-watcher goroutine, and the device itself. The replacement would then have failed to open the same device, and the node would have quietly stopped being able to play after its first reconfiguration.

The whole test suite was green, and could not have been anything else. Every test, including the end-to-end vertical slice through a real broker and the real agent binary, runs against `fakesink`. A `fakesink` holds no device, so the leak has no symptom there. The test environment differs from the deployment environment in exactly the dimension the defect lives in, which is the same shape as this project's Node-versus-browser `fetch` finding and its macOS-versus-Linux socket finding, in a third disguise.

**Rule:** when a component can be replaced at runtime, ask what the outgoing instance still holds that is exclusive, and give it an explicit release rather than assuming garbage collection covers it. A device, a socket, a port, a lock, and a file handle are all exclusive; a test double for any of them is not, which is why no fake-backed test can raise this class of defect. Hand the replaced instance back to the caller rather than dropping it inside the swap, so releasing it is a visible obligation instead of an omission nobody can see.

## A dependency's own dependencies pin a platform floor nobody wrote down, and the development machine is the last place it shows

**Track C, 2026-08-20.** The audio agent's GStreamer bindings pull in `go-glib`, which calls GLib symbols added as recently as 2.80. Nothing in this project stated a GLib requirement, because nothing had to: the development machine installs GStreamer through Homebrew, which carries a current GLib, and every gate was green there for as long as the engine existed. Building the same code on Debian 12, which ships GLib 2.74, fails outright with twenty undefined C symbols. It builds and passes on Debian 13 at GLib 2.84.

The floor was never a decision anyone made. It arrived through a transitive dependency, and the only way to learn it was to compile on the platform the software actually deploys to. The same run found a second fact of the same kind: the production output sink lives in its own distribution package that neither CI nor the recorded node package set installed, so the agent would have started, accepted its configuration, and then had no sink.

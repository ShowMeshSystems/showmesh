...
// Mosquitto container underneath a running coordinator subprocess and
// checks two independent things that only a genuinely re-established
// subscription (not merely a reconnected TCP session) can produce: a live
// heartbeat delivered strictly after the restart, and a subsequent Last
// Will still being observed.
//
...
func TestBrokerRestartResubscribesAndObservesSubsequentChanges(t *testing.T) {
	...
	// coord.ready is this suite's post-Step-3 replacement for direct access
	// to a live *broker.BrokerManager (coord.bm.State().Connected no longer
	// exists once the coordinator is a subprocess): /readyz reports exactly
	// the same underlying fact, through the coordinator's own intentional
	// external interface — see testCoordinator.ready's doc comment.
	waitFor(t, 30*time.Second, 200*time.Millisecond, func() bool {
		return coord.ready(t)
	}, "coordinator's /readyz to report ready again after the broker restart")
	...
	waitFor(t, 30*time.Second, 50*time.Millisecond, func() bool {
		...
		observedAt := parseObservedAt(t, v.Evidence.Heartbeat.ObservedAt)
		return observedAt != nil && observedAt.After(restartAt)
	...
	// "offline" verdict, not merely "not online").
	agent.sigkill(t)
